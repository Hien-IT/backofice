...
import { generateExtensionsEntrypoint, resolveFsExtensions, resolveModuleExtensions } from '@backoffice/extensions/node';
...
export default defineConfig({
	...
	plugins: [
		...
		{
			name: 'watch-backoffice-dependencies',
			configureServer: (server) => {
				server.watcher.options = {
					...server.watcher.options,
					ignored: [/node_modules\/(?!@backoffice\/).*/, '**/.git/**'],
				};
			},
		},
		vueDevtools(),
...
function backofficeExtensions() {
	const virtualExtensionsId = '@backoffice-extensions';

	let extensionsEntrypoint = null;

	return [
		{
			name: 'backoffice-extensions-serve',
			apply: 'serve',
			config: () => ({
				optimizeDeps: {
		...
		{
			name: 'backoffice-extensions-build',
			apply: 'build',
			config: () => ({
				build: {
					rollupOptions: {
