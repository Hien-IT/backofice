...
import { useAppStore } from '@backoffice/stores';
import type { ShowSelect } from '@backoffice/types';
...
const boxSelectControl = new BoxSelectControl({
	...
	layers: ['__backoffice_polygons', '__backoffice_points', '__backoffice_lines'],
});
...
function setupMap() {
	...
	map.on('load', () => {
		...
		const activeLayers = ['__backoffice_polygons', '__backoffice_points', '__backoffice_lines'];
		...
		map.on('click', '__backoffice_clusters', expandCluster);
		map.on('mousemove', '__backoffice_clusters', hoverCluster);
		map.on('mouseleave', '__backoffice_clusters', hoverCluster);
...
function updateData(newData: any) {
	const source = map.getSource('__backoffice');
...
function updateSource(newSource: GeoJSONSource) {
	...
	if (map.getStyle().sources?.['__backoffice']) {
		map.removeSource('__backoffice');
	}

	map.addSource('__backoffice', { ...newSource, data: props.data });
...
function updateSelection(newSelection?: (string | number)[], previousSelection?: (string | number)[]) {
	previousSelection?.forEach((id) => {
		map.setFeatureState({ id, source: '__backoffice' }, { selected: false });
		map.removeFeatureState({ id, source: '__backoffice' });
	});

	newSelection?.forEach((id) => {
		map.setFeatureState({ id, source: '__backoffice' }, { selected: true });
	});
}
...
function updatePopup(event: MapLayerMouseEvent) {
	...
	if (previousId && featureChanged) {
		map.setFeatureState({ id: previousId, source: '__backoffice' }, { hovered: false });
	}
	...
	if (feature && feature.properties) {
		...
		if (featureChanged) {
			map.setFeatureState({ id: feature.id, source: '__backoffice' }, { hovered: true });
...
function expandCluster(event: MapLayerMouseEvent) {
	const features = map.queryRenderedFeatures(event.point, {
		layers: ['__backoffice_clusters'],
	});

	const clusterId = features[0]?.properties?.cluster_id;
	const source = map.getSource('__backoffice') as GeoJSONSource;
