import { useCollection, useFilterFields, useItems, useSync } from '@backoffice/composables';
import { defineLayout } from '@backoffice/extensions';
import { Field, PermissionsAction, User } from '@backoffice/types';
import { getEndpoint, getRelationType, moveInArray } from '@backoffice/utils';
	...
	setup(props, { emit }) {
				...
				if (junction !== undefined) {
					const related = relationsStore.relations.find(
						(relation) =>
							...
							relation.related_collection === 'backoffice_users',
					);
			...
			file: (field) => {
				...
				const relation = relationsStore.relations.find((relation) => {
					return (
						relation.collection === props.collection &&
						relation.field === field.field &&
						relation.related_collection === 'backoffice_files'
					);
				});
		...
		function useLayoutQuery() {
			...
			const fields = computed<string[]>(() => {
				...
				if (props.collection === 'backoffice_files' && imageSource.value === '$thumbnail') {
