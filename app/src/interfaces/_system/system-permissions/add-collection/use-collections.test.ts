...
describe('useCollections', () => {
	...
	describe('systemCollections', () => {
		it('returns only system collections', () => {
			...
			collectionsStore.collections = [
				makeCollection('articles'),
				makeCollection('backoffice_users'),
				makeCollection('backoffice_files'),
			];

			const { systemCollections } = useCollections({ excludeCollections: [] });

			expect(systemCollections.value).toHaveLength(2);
			expect(systemCollections.value.map((c) => c.collection).sort()).toEqual(['backoffice_files', 'backoffice_users']);
		});
		...
		it('orders system collections alphabetically', () => {
			...
			collectionsStore.collections = [
				makeCollection('backoffice_users'),
				makeCollection('backoffice_activity'),
				makeCollection('backoffice_files'),
			];
		...
		it('marks excluded system collections as disabled', () => {
			...
			collectionsStore.collections = [makeCollection('backoffice_users'), makeCollection('backoffice_files')];

			const { systemCollections } = useCollections({ excludeCollections: ['backoffice_users'] });

			const users = systemCollections.value.find((c) => c.collection === 'backoffice_users');
			const files = systemCollections.value.find((c) => c.collection === 'backoffice_files');
	...
	describe('displayItems', () => {
		it('includes both user and system collections', () => {
			...
			collectionsStore.collections = [makeCollection('articles'), makeCollection('backoffice_users')];
			...
			expect(collectionNames).toContain('backoffice_users');
		});
