...
const options = computed(() => {
	let options: any[] = [
		...
		},
		{
			...
			value: 'backoffice_users',
		},
		{
			text: t('role') + ': ' + t('select'),
			value: 'backoffice_roles',
		},
	];
...
function onSelectItem(value: (string | number)[] | null) {
	if (collection.value === 'backoffice_users') return emit('input', 'user_' + value![0]);
	if (collection.value === 'backoffice_roles') return emit('input', 'role_' + value![0]);
}
