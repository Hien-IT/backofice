...
import type { ContentVersion, Filter } from '@backoffice/types';
import { deepMap, getFieldsFromTemplate } from '@backoffice/utils';
...
const downloadName = computed(() => {
	if (relatedPrimaryKey.value === null || relationInfo.value?.relatedCollection.collection !== 'backoffice_files') return;
	const junctionField = relationInfo.value.junctionField.field;
	const relationPkField = relationInfo.value.relatedPrimaryKeyField.field;
		...
		>
			<template #actions>
				<PrivateViewHeaderBarActionButton
					v-if="currentlyEditing !== '+' && relationInfo.relatedCollection.collection === 'backoffice_files'"
					icon="download"
					secondary
