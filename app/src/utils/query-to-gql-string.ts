...
import { Filter, Query } from '@backoffice/types';
...
export function formatQuery({ collection, query }: QueryInfo): Record<string, any> {
	...
	const alias = isSystemCollection(collection) ? collection.substring(11) : collection;
...
/**
 * Replace functions from Backoffice-Filter format to GraphQL format
 */
