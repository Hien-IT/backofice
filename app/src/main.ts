...
import { BACKOFFICE_LOGO } from './constants';
import { registerDirectives } from './directives/register';
import { loadExtensions, registerExtensions } from './extensions';
import { i18n } from './lang/';
import { router } from './router';
import './styles/main.scss';
import '@backoffice/vue-split-panel/index.css';
...
async function init() {
	console.log(BACKOFFICE_LOGO);

	console.info(
		`Hey! Interested in helping build this open-source data management platform?\nIf so, join our growing team of contributors at: https://backoffice.chat`,
	);

	console.info(`%c Starting Backoffice...`, 'color:Green');
