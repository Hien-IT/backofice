...
const { createAllowed, updateAllowed, deleteAllowed } = useCollectionPermissions('backoffice_dashboards');
...
function exportDashboard(ids: string[]) {
	const endpoint = getEndpoint('backoffice_dashboards');
...
<template>
	<PrivateView :title="$t('insights')" icon="insights">
		...
		<template #sidebar>
			<BasicImportSidebarDetail collection="backoffice_dashboards" @refresh="refresh" />
		</template>
