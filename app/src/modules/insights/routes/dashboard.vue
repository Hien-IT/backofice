...
import { applyOptionsData } from '@backoffice/utils';
...
const { updateAllowed } = useItemPermissions('backoffice_panels', props.primaryKey, false);
...
<template>
	...
	<PrivateView v-else :title="currentDashboard.name" :icon="currentDashboard.icon">
		...
		<template #sidebar>
			<CommentsSidebarDetail :key="primaryKey" collection="backoffice_dashboards" :primary-key="primaryKey" />
