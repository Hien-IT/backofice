...
import { getDateTimeFormatted, mergeFilters } from '@backoffice/utils';
...
const { layout, layoutOptions, layoutQuery, filter, search, resetPreset } = usePreset(ref('backoffice_files'));
...
const {
	...
	createAllowed,
} = useCollectionPermissions('backoffice_files');

const { createAllowed: createFolderAllowed, deleteAllowed: folderDeleteAllowed } =
	useCollectionPermissions('backoffice_folders');
...
<template>
	...
	>
		<PrivateView :title="title" icon="folder" :class="{ dragging }">
			...
			<template #actions>
				<SearchInput v-model="search" v-model:filter="filter" collection="backoffice_files" small />
