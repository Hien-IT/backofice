...
import type { Field, File } from '@backoffice/types';
...
const {
	...
	getItem,
	validationErrors,
} = useItem<File>(ref('backoffice_files'), primaryKey);
...
const {
	...
	focused,
	connectionId,
} = useCollab(ref('backoffice_files'), primaryKey, ref(null), item, edits, getItem);
	...
	<PrivateView v-else :title="loading || !item ? $t('loading') : item.title" show-back back-to="/files">
		...
		<template #sidebar>
			...
			<RevisionsSidebarDetail
				...
				@revert="revert"
			/>
			<CommentsSidebarDetail v-if="isNew === false" collection="backoffice_files" :primary-key="primaryKey" />
		</template>
	</PrivateView>
</template>
