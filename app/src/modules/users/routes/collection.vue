...
import { useLayout } from '@backoffice/composables';
import { mergeFilters } from '@backoffice/utils';
...
const { layout, layoutOptions, layoutQuery, filter, search, resetPreset } = usePreset(ref('backoffice_users'));
...
} = useCollectionPermissions('backoffice_users');

const { readAllowed: rolesReadAllowed } = useCollectionPermissions('backoffice_roles');
...
<template>
	...
	>
		<PrivateView :title="title" icon="people_alt">
			...
			<template #actions>
				<SearchInput v-model="search" v-model:filter="filter" collection="backoffice_users" small />
