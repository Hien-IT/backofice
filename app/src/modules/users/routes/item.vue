...
import { useShortcut } from '@backoffice/composables';
import type { User } from '@backoffice/types';
...
const { info: collectionInfo } = useCollection('backoffice_users');
...
const {
	...
	refresh,
	getItem,
} = useItem<User>(ref('backoffice_users'), primaryKey, null, {
...
const {
	...
	focused,
	connectionId,
} = useCollab(ref('backoffice_users'), primaryKey, ref(null), item, edits, getItem);
	...
	<PrivateView :title="title" show-back back-to="/users">
		...
		<template #sidebar>
			...
			<RevisionsSidebarDetail
				...
				@revert="revert"
			/>
			<CommentsSidebarDetail v-if="isNew === false" collection="backoffice_users" :primary-key="primaryKey" />
		</template>
	</PrivateView>
</template>
