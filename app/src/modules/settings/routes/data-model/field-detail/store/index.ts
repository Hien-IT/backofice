import type { DisplayConfig, InterfaceConfig } from '@backoffice/extensions';
import type { Collection, DeepPartial, Field, LocalType, Relation } from '@backoffice/types';
import { getEndpoint } from '@backoffice/utils';
		...
		async startEditing(collection: string, field: string, localType?: LocalType) {
			...
			if (field !== '+') {
				...
				if (['files', 'm2m', 'translations', 'm2a'].includes(this.localType)) {
					// These types rely on backoffice_relations fields being said, so meta should exist for these particular relations
		...
		update(updates: DeepPartial<typeof this.$state>) {
			...
			this.$patch((state) => {
				...
				if (hasChanged('relations.m2o.related_collection')) {
					...
					if (get(updates, 'relations.m2o.related_collection') === 'backoffice_files') {
