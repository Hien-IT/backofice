...
function useOnCreate() {
	...
	const onCreateOptions = computed(() => {
		if (type.value === 'uuid') {
			...
			if (localType.value === 'm2o' && relations.value.m2o?.related_collection === 'backoffice_users') {
				return options.filter(({ value }) => [null, 'user-created'].includes(value));
			}

			if (localType.value === 'm2o' && relations.value.m2o?.related_collection === 'backoffice_roles') {
