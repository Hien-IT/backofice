...
const {
	...
	isSingleton,
	accountabilityScope,
} = useCollection('backoffice_translations');
...
const {
	...
	refresh,
	validationErrors,
} = useItem(ref('backoffice_translations'), primaryKey);
...
<template>
	...
	>
		...
		<template #navigation>
			<SettingsNavigation current-collection="backoffice_translations" />
		</template>
		...
		<VForm
			...
			:loading="loading"
			:initial-values="item"
			collection="backoffice_translations"
		...
		<template #sidebar>
			<template v-if="isNew === false && loading === false && internalPrimaryKey">
				<RevisionsSidebarDetail
					...
					@revert="revert"
				/>
				<CommentsSidebarDetail collection="backoffice_translations" :primary-key="internalPrimaryKey" />
			</template>
		</template>
	</PrivateView>
</template>
