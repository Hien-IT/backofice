...
import { useCollection, useLayout } from '@backoffice/composables';
...
const { info: currentCollection } = useCollection('backoffice_translations');
...
<template>
	...
	>
		<PrivateView :title="$t('settings_translations')" icon="translate">
			...
			<template #actions>
				<SearchInput v-model="search" v-model:filter="filter" collection="backoffice_translations" small />
