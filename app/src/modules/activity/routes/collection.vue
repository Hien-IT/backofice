...
import { Filter } from '@backoffice/types';
...
const { layout, layoutOptions, layoutQuery, filter, search } = usePreset(ref('backoffice_activity'));
...
<template>
	...
	>
		<PrivateView :title="$t('activity_feed')" icon="access_time">
			...
			<template #actions>
				<SearchInput v-model="search" v-model:filter="filter" collection="backoffice_activity" />
			</template>
