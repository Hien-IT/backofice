...
import type { Field, PrimaryKey } from '@backoffice/types';
...
function useFile() {
	const isBackofficeFiles = computed(() => {
		return collection === 'backoffice_files' || relatedCollection === 'backoffice_files';
	});

	const file = computed(() => {
		if (isBackofficeFiles.value === false) return null;
