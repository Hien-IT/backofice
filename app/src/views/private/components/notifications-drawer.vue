...
import { useItems } from '@backoffice/composables';
import { useAppStore } from '@backoffice/stores';
import { Filter, Notification } from '@backoffice/types';
...
watch(notificationsDrawerOpen, (open) => {
	...
	collection.value = open ? 'backoffice_notifications' : null;
});
...
<template>
	...
	>
		...
		<template #actions>
			<SearchInput v-model="search" v-model:filter="filter" collection="backoffice_notifications" />
