import { useCollection } from '@backoffice/composables';
import { isSystemCollection } from '@backoffice/system-data';
import { Alterations, Field, Item, PrimaryKey, Query, Relation } from '@backoffice/types';
import { getEndpoint, isObject } from '@backoffice/utils';
...
export function useItem<T extends Item>(
	...
	async function saveAsCopy() {
		...
		const alias = isSystemCollection(collection.value) ? collection.value.substring(11) : collection.value;
	...
	function setItemValueToResponse(response: T) {
		if (
			(isSystemCollection(collection.value) && collection.value !== 'backoffice_collections') ||
			(collection.value === 'backoffice_collections' && isSystemCollection(response.collection ?? ''))
		) {
