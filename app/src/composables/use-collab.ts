import { ErrorCode } from '@backoffice/errors';
import { BackofficeUser, readUser, readUsers, realtime, RemoveEventHandler, WebSocketClient } from '@backoffice/sdk';
import {
	ACTION,
	Avatar,
	ClientID,
...
import { isDetailedUpdateSyntax, isObject } from '@backoffice/utils';
 ...
 * TODO: Remove once https://linear.app/backoffice/issue/CMS-1702 is done
 */
type CollabUserInfo = {
	id: string;
		...
		if (message.users.length > 0) {
			const usersInfo = await sdk.request(
				readUsers({
					...
					// TODO: Update this once https://github.com/backoffice/backoffice/issues/26558 is Done
					fields: ['id', 'first_name', 'last_name', { avatar: ['id', 'modified_on'] }] as (keyof BackofficeUser)[],
				}),
			);
