...
import { createBackoffice, createFlow, createOperation, rest, staticToken, updateFlow } from '@backoffice/sdk';
...
test('syncronized flow logging', { timeout: 120_000 }, async () => {
	...
	const backoffice = await sandbox(database, {
		...
		env: {
			...
			SYNCHRONIZATION_NAMESPACE: `backoffice-${database}`,
			DB_FILENAME: `backoffice_test_${getUID()}.db`,
		},
	});

	const api = createBackoffice<unknown>(`http://localhost:${backoffice.apis[0].port}`)
		.with(rest())
	...
	backoffice.logger.onLog((msg, group) => {
