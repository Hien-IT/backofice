import { createBackoffice, createItem, deleteItem, realtime, rest, staticToken, updateItem } from '@backoffice/sdk';
import { options, port } from '@utils/constants.js';
import { useSnapshot } from '@utils/use-snapshot.js';
import { expect, test } from 'vitest';
import type { Schema } from './schema.d.ts';

const api = createBackoffice<Schema>(`http://localhost:${port}`).with(realtime()).with(rest()).with(staticToken('admin'));
await api.connect();
