...
test('get permissions for user', async () => {
	...
	const userApi = createBackoffice<Schema>(`http://localhost:${port}`).with(rest()).with(staticToken(token));
...
test('get permissions without auth', async () => {
	const publicApi = createBackoffice<Schema>(`http://localhost:${port}`).with(rest());
