import { createBackoffice, graphql, rest, staticToken } from '@backoffice/sdk';
...
const api = createBackoffice<Schema>(`http://localhost:${port}`).with(graphql()).with(rest()).with(staticToken('admin'));

const { snapshot } = await useSnapshot<Schema>(api);

// TODO: Oracle has a **STUPID** hard limit of VARCHAR(4000) on backoffice_revisions.data, so this currently fails to generate the scoped user
if (database !== 'oracle')
