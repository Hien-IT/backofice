# Backoffice e2e testing suite

These E2E tests are to test the whole of Backoffice at once. Ensuring that all components (e.g. API, UI, DB and other
...
- `pnpm test` or `pnpm vitest --project sqlite` to start up the tests against backoffice running with a sqlite database
...
It is recommended to use the Backoffice SDK in most cases to use and test functionality of the API, alternatively make use
...
### Testing against a custom Backoffice instance

In order to test Backoffice against custom configurations like different env vars, a test can spin up their own Backoffice
...
test('Pinging the API', async () => {
	...
	const api = createBackoffice(`http://localhost:${backoffice.apis[0].port}`).with(rest()).with(staticToken('admin'));
