...
program
	.name('sandbox')
	.description('CLI for spinning up backoffice sandboxes for testing and development purposes')
	.version('1.0.0')
	.addArgument(new Argument('<database>', 'What database to start the api with').choices(databases))
	.option('-b, --build', 'Rebuild backoffice from source')
	.option('-d, --dev', 'Start backoffice in developer mode. Not compatible with build')
	...
	.option('-i, --instances <instances>', 'Horizontally scale backoffice to a given number of instances', '1')
