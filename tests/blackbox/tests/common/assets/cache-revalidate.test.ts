...
describe('/assets cache control', () => {
	...
	beforeAll(async () => {
		...
		for (const vendor of vendors) {
			...
			backofficeInstances[vendor] = [serverRevalidate, serverDefault];
			envs[vendor] = { envRevalidate, envDefault };

			promises.push(awaitBackofficeConnection(portRevalidate), awaitBackofficeConnection(portDefault));
		}
