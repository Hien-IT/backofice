...
describe('Flows Schedule Hook Tests', () => {
	...
	const backofficeInstances = {} as Record<Vendor, ChildProcess[]>;
	...
	beforeAll(async () => {
		...
		for (const vendor of vendors) {
			...
			envRedis1[vendor]['SYNCHRONIZATION_NAMESPACE'] = `backoffice-${vendor}`;
			...
			backofficeInstances[vendor] = [server1, server2, server3];
			envs[vendor] = [envRedis1, envRedis2, envMemory];

			promises.push(
				awaitBackofficeConnection(newServerPort1),
				awaitBackofficeConnection(newServerPort2),
				awaitBackofficeConnection(newServerPort3),
			);
		}
