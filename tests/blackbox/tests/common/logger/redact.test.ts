...
describe('Logger Redact Tests', () => {
	...
	beforeAll(async () => {
		...
		for (const vendor of vendors) {
			...
			backofficeInstances[vendor] = server;

			promises.push(awaitBackofficeConnection(newServerPort));
		}
	...
	afterAll(async () => {
		for (const [vendor, connection] of databases) {
			backofficeInstances[vendor].kill();
			...
			TEST_USERS.forEach((userKey) => {
				describe(USER[userKey].NAME, () => {
					it.each(vendors)('%s', async (vendor) => {
						...
						// Action
						const logger = new TestLogger(backofficeInstances[vendor], '/auth/refresh', true);
						...
						const loggerGql = new TestLogger(backofficeInstances[vendor], '/graphql/system', true);
