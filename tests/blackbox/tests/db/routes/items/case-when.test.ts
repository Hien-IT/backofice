...
describe('retrieves items with filters', async () => {
	...
	it.each(vendors)('%s', async (vendor) => {
		...
		// user_created is a string, whereas backoffice_users.id is a of type UUID.
		const comparisonResult = await request(getUrl(vendor))
			.get(
				`/comments?filter[_and][0][collection][_eq]=backoffice_users&filter[_and][1][item][_eq]=${userResponse.body.data.id}&aggregate[count]=id`,
			)
			.set('Authorization', `Bearer ${userToken}`)
			.expect(200);
