...
import type { Collection } from '@backoffice/types';
	...
	describe(`pkType: ${pkType}`, () => {
		...
		describe('POST /', () => {
			...
			afterEach(async () => {
				...
				await db('backoffice_collections').del().where({ collection: TEST_COLLECTION_NAME });
			});
		...
		describe('PATCH /', () => {
			...
			afterEach(async () => {
				...
				for (const collection of collectionNames) {
					...
					await db('backoffice_collections').del().where({ collection });
				}
			});
				...
				TEST_USERS.forEach((userKey) => {
					describe(USER[userKey].NAME, () => {
						it.each(vendors)('%s', async (vendor) => {
							...
							if (userKey === USER.ADMIN.KEY) {
								expect(await db('backoffice_collections').select().where({ collection: TEST_FOLDER_NAME })).toHaveLength(
									1,
								);
							}
