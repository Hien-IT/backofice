...
describe('Schema Caching Tests', () => {
	describe('GET /collections/:collection', () => {
		describe('schema change propagates across nodes using messenger', () => {
			...
			const tzBackoffice = {} as { [vendor: string]: ChildProcess[] };
			...
			beforeAll(async () => {
				...
				for (const vendor of vendors) {
					...
					env1[vendor]['CACHE_NAMESPACE'] = 'backoffice-schema-cache';
					...
					tzBackoffice[vendor] = [server1, server2];
					envs[vendor] = [env1, env2];

					promises.push(awaitBackofficeConnection(newServerPort1), awaitBackofficeConnection(newServerPort2));
				}
			...
			afterAll(async () => {
				for (const [vendor, connection] of databases) {
					for (const instance of tzBackoffice[vendor]!) {
						instance.kill();
					}
		...
		describe('schema change does not propagate across nodes without messenger', () => {
			...
			beforeAll(async () => {
				...
				for (const vendor of vendors) {
					...
					const cacheNamespacePrefix = `backoffice-${vendor}`;
					...
					tzBackoffice[vendor] = [server3, server4];
					envs[vendor] = [env3, env4];

					promises.push(awaitBackofficeConnection(newServerPort3), awaitBackofficeConnection(newServerPort4));
				}
