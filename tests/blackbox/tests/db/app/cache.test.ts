...
import { awaitBackofficeConnection } from '@utils/await-connection';
...
describe('App Caching Tests', () => {
	...
	const backofficeInstances = {} as { [vendor: string]: ChildProcess[] };
	...
	const cacheNamespacePrefix = 'backoffice-app-cache';
	...
	beforeAll(async () => {
		...
		for (const vendor of vendors) {
			...
			envMem[vendor]['CACHE_AUTO_PURGE_IGNORE_LIST'] = `backoffice_activity,backoffice_presets,${collectionIgnored}`;
			...
			backofficeInstances[vendor] = [serverMem, serverMemPurge, serverRedis, serverRedisPurge];
			envs[vendor] = { envMem, envMemPurge, envRedis, envRedisPurge };

			promises.push(
				awaitBackofficeConnection(newServerPortMem),
				awaitBackofficeConnection(newServerPortMemPurge),
				awaitBackofficeConnection(newServerPortRedis),
				awaitBackofficeConnection(newServerPortRedisPurge),
			);
		}
	...
	afterAll(async () => {
		for (const [vendor, connection] of databases) {
			for (const instance of backofficeInstances[vendor]!) {
				instance.kill();
			}
