...
import { ForbiddenError, InvalidPayloadError, ServiceUnavailableError } from '@backoffice/errors';
import { ClientMessage, type WebSocketClient, WS_TYPE } from '@backoffice/types';
	...
	bindWebSocket() {
		...
		this.busHandler = (event: any) => {
			...
			this.eventQueue = this.eventQueue
				.then(async () => {
					if (
						event.collection === 'backoffice_settings' &&
						event.action === 'update' &&
					...
					if (event.action === 'update' || event.action === 'delete') {
						...
						const roomsToUpdate = Object.values(this.roomManager.rooms).filter((room) => {
							// Versioned Rooms
							if (room.version) {
								return event.collection === 'backoffice_versions' && keys.some((key) => String(key) === room.version);
							}

							// Skip non-matching collections and version events
							if (room.collection !== event.collection || event.collection === 'backoffice_versions') return false;
				...
				const { accessAllowed: versionAccessAllowed } = await validateItemAccess(
					{
						accountability: client.accountability!,
						action: 'read',
						collection: 'backoffice_versions',
