...
import type { WebSocketClient } from '@backoffice/types';
...
describe('room', () => {
	...
	test('broadcasts save action for system collections', async () => {
		const clientA = mockWebSocketClient({ uid: 'abc' });
		const item = getTestItem();
		const uid = getRoomHash('backoffice_users', item, null);
		const room = new Room(uid, 'backoffice_users', item, null, {}, mockMessenger);
		await room.ensureInitialized();
		...
		await room.onUpdateHandler({ keys: [item], collection: 'backoffice_users' }, {
			accountability: { user: 'external-user' },
		} as any);
...
describe('versioned room', () => {
	...
	test('onDeleteHandler triggers for matching version', async () => {
		...
		await room.onDeleteHandler({ keys: [versionId], collection: 'backoffice_versions' }, {} as any);
