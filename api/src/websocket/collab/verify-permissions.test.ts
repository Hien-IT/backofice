...
describe('PermissionCache Invalidation', () => {
	test('Schema change invalidates cache', async () => {
		...
		const bus = useBus();
		await bus.publish('websocket.event', { collection: 'backoffice_fields' });
	...
	test('Collection metadata change invalidates cache', async () => {
		...
		const bus = useBus();
		await bus.publish('websocket.event', { collection: 'backoffice_collections' });
	...
	test('User-specific dependency invalidation', async () => {
		...
		const bus = useBus();
		await bus.publish('websocket.event', { collection: 'backoffice_users', key: 'user-a' });
