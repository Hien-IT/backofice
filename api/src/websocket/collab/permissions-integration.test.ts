...
describe('Permissions Integration', () => {
	...
	beforeEach(async () => {
		...
		vi.mocked(getSchema).mockResolvedValue({
			collections: {
				...
				backoffice_users: { primary: 'id', fields: { id: { type: 'uuid' }, department: { type: 'uuid' } } },
			...
			relations: [
				{ collection: 'articles', field: 'author', related_collection: 'backoffice_users' },
				{ collection: 'backoffice_users', field: 'department', related_collection: 'departments' },
	...
	const testCases = [
		...
		},
		{
			...
			itemData: { id: 1 },
			expectedFields: ['*'],
			verifyTags: ['backoffice_users:user-uuid', 'departments', 'teams'],
		},
		{
	...
	test('$CURRENT_USER update', async () => {
		...
		const bus = useBus();
		await bus.publish('websocket.event', { collection: 'backoffice_users', keys: ['user-abc'] });
	...
	test('System-level change', async () => {
		...
		const bus = useBus();
		await bus.publish('websocket.event', { collection: 'backoffice_permissions' });
