...
describe('PermissionCache', () => {
	...
	describe('Invalidation', () => {
		...
		test('dependency invalidation', () => {
			...
			cache.set(mockAccountability, 'posts', '1', 'read', ['A'], ['backoffice_users:123']);
		...
		test('system collection invalidation clears everything', () => {
			...
			busHandler({ collection: 'backoffice_permissions' });
		...
		test('wildcard dependency invalidation (no keys provided)', () => {
			...
			// Invalidate backoffice_users without keys (e.g. from Flow)
			busHandler({ collection: 'backoffice_users' });
		...
		test('specific dependency invalidation does NOT invalidate unrelated specific dependencies', () => {
			...
			cache.set(mockAccountability, 'posts', '2', 'read', ['B'], ['backoffice_users:456']);

			// Invalidate user 456
			busHandler({ collection: 'backoffice_users', keys: ['456'] });
		...
		test('deeply nested mapping tags correctly (user update clears user dependency but not others)', () => {
			cache.set(mockAccountability, 'articles', '1', 'read', ['*'], ['backoffice_users:user-1']);
			const accountB = { ...mockAccountability, user: 'user-2' };
			cache.set(accountB, 'articles', '2', 'read', ['*'], ['backoffice_users:user-2']);

			busHandler({ collection: 'backoffice_users', keys: ['user-2'] });
		...
		test('dynamic variable $CURRENT_ROLE invalidation', () => {
			cache.set(mockAccountability, 'posts', '1', 'read', ['*'], ['backoffice_roles:role-1']);

			busHandler({ collection: 'backoffice_roles', keys: ['role-1'] });
		...
		test('dynamic variable $CURRENT_ROLES invalidation', () => {
			...
			cache.set(accMultiRole, 'posts', '1', 'read', ['*'], ['backoffice_roles:role-A', 'backoffice_roles:role-B']);

			busHandler({ collection: 'backoffice_roles', keys: ['role-B'] });
		...
		test('deeply nested dynamic path invalidation ($CURRENT_USER.dept.team)', () => {
			cache.set(mockAccountability, 'articles', '1', 'read', ['*'], ['backoffice_users:user-1', 'departments', 'teams']);

			// User update clears it
			expect(cache.get(mockAccountability, 'articles', '1', 'read')).toBeDefined();
			busHandler({ collection: 'backoffice_users', keys: ['user-1'] });
	...
	describe('Security & Edge Cases', () => {
		test('item dependent on multiple sources invalidates if ANY source changes', () => {
			cache.set(mockAccountability, 'posts', '1', 'read', ['A'], ['backoffice_users:A', 'backoffice_files:B']);

			// Invalidate User A -> Cache should clear
			busHandler({ collection: 'backoffice_users', keys: ['A'] });
			expect(cache.get(mockAccountability, 'posts', '1', 'read')).toBeUndefined();

			cache.set(mockAccountability, 'posts', '1', 'read', ['A'], ['backoffice_users:A', 'backoffice_files:B']);

			// Invalidate File B -> Cache should clear
			busHandler({ collection: 'backoffice_files', keys: ['B'] });
			expect(cache.get(mockAccountability, 'posts', '1', 'read')).toBeUndefined();
		});

		test('overlapping dependencies: multiple items depend on same source', () => {
			cache.set(mockAccountability, 'posts', '1', 'read', ['A'], ['backoffice_users:X']);
			cache.set(mockAccountability, 'posts', '2', 'read', ['B'], ['backoffice_users:X']);

			busHandler({ collection: 'backoffice_users', keys: ['X'] });
		...
		test('handles empty keys array same as missing keys (generic invalidation)', () => {
			...
			busHandler({ collection: 'backoffice_users', keys: [] });
		...
		test('system collection invalidation wipes entire cache rigorously', () => {
			...
			busHandler({ collection: 'backoffice_roles' });
		...
		test('system collection invalidation ignores keys and wipes everything anyway', () => {
			...
			busHandler({ collection: 'backoffice_permissions', keys: ['some-id'] });
