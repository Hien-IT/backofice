import { systemCollectionNames } from '@backoffice/system-data';
...
export function registerWebSocketEvents() {
	...
	registerActionHooks([
		...
		...systemCollectionNames
			...
			.map((collection) => collection.replace('backoffice_', '')),
	]);
...
function registerFieldsHooks() {
	// exception for field hooks that don't report `backoffice_fields` as being the collection
...
function registerRelationsHooks() {
	// exception for relation hooks that don't report `backoffice_relations` as being the collection
