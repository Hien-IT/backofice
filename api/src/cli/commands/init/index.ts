...
export default async function init(): Promise<void> {
	...
	await db('backoffice_roles').insert({ ...defaultAdminRole, id: role });
	await db('backoffice_policies').insert({ ...defaultAdminPolicy, id: policy });
	await db('backoffice_access').insert({ id: randomUUID(), role, policy });

	await db('backoffice_users').insert({
		...defaultAdminUser,
		id: randomUUID(),
	...
	process.stdout.write(`\nStart Backoffice by running:\n`);
	process.stdout.write(`  ${chalk.blue('cd')} ${rootPath}\n`);
	process.stdout.write(`  ${chalk.blue('npx backoffice')} start\n`);
