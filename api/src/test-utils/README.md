...
Sets up default CRUD operation mocks for all Backoffice system collections (backoffice_collections, backoffice_fields, etc.).
...
Provides schema inspector mocking utilities for the `@backoffice/schema` package.
...
```typescript
// Standard usage
vi.mock('@backoffice/schema', async () => {
...
Provides environment variable mocking utilities for the `@backoffice/env` package.
...
Provides storage driver and storage manager mocking utilities for the `@backoffice/storage` package.
...
```typescript
import { getStorage } from '../storage/index.js';
import type { Driver, StorageManager } from '@backoffice/storage';
import { createMockDriver, createMockStorage } from '../test-utils/storage.js';
...
Creates a mock Express Request pre-populated with common Backoffice properties (`accountability`, `schema`,
`sanitizedQuery`, etc.).
...
```typescript
import { SchemaBuilder } from '@backoffice/schema-builder';
...
describe('Service Tests', () => {
	...
	test('should query backoffice_fields', async () => {
		// Override the default empty response for specific tests
		tracker.on.select('backoffice_fields').response([{ id: 1, collection: 'users', field: 'name' }]);
