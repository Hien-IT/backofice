import type { Snapshot } from '@backoffice/types';
import { describe, expect, test, vi } from 'vitest';
import { validateSnapshot } from './validate-snapshot.js';

vi.mock('backoffice/version', () => ({
	version: '10.0.0',
}));
...
describe('should fail on invalid snapshot schema', () => {
	...
	test('invalid schema', () => {
		const snapshot = { version: 1, backoffice: '10.0.0', collections: {} } as Snapshot;

		expect(() => validateSnapshot(snapshot)).toThrowError('"collections" must be an array');
	});

	test('invalid systemFields schema', () => {
		const snapshot = { version: 1, backoffice: '10.0.0', vendor: 'sqlite', systemFields: {} } as Snapshot;
...
describe('should require force option on version / vendor mismatch', () => {
	test('backoffice version mismatch', () => {
		const snapshot = { version: 1, backoffice: '9.26.0' } as Snapshot;

		expect(() => validateSnapshot(snapshot)).toThrowError(
			"Provided snapshot's backoffice version 9.26.0 does not match the current instance's version 10.0.0",
		);
	});

	test('missing vendor property', () => {
		const snapshot = { version: 1, backoffice: '10.0.0' } as Snapshot;
	...
	test('db vendor mismatch', () => {
		const snapshot = { version: 1, backoffice: '10.0.0', vendor: 'postgres' } as Snapshot;
...
test('should allow bypass on version / vendor mismatch via force option ', () => {
	const snapshot = { version: 1, backoffice: '9.26.0', vendor: 'postgres' } as Snapshot;
