import { InvalidTokenError, ServiceUnavailableError, TokenExpiredError } from '@backoffice/errors';
...
export function verifyAccessJWT(token: string, secret: string) {
	const payload = verifyJWT(token, secret) as BackofficeTokenPayload;
