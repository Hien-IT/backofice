import { InvalidTokenError, TokenExpiredError } from '@backoffice/errors';
import { ServiceUnavailableError } from '@backoffice/errors';
import jwt from 'jsonwebtoken';
import { expect, test, vi } from 'vitest';
import type { BackofficeTokenPayload } from '../types/index.js';
import { verifyAccessJWT, verifyJWT } from './jwt.js';

const payload: BackofficeTokenPayload = { role: null, app_access: false, admin_access: false };
const secret = 'test-secret';
const options = { issuer: 'backoffice' };
...
test('Returns the payload of a correctly signed token', () => {
	...
	expect(result['iss']).toBe('backoffice');
...
const RequiredEntries: Array<keyof BackofficeTokenPayload> = ['role', 'app_access', 'admin_access'];
