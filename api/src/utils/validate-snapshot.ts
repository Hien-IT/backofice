...
const snapshotJoiSchema = Joi.object({
	...
	backoffice: Joi.string().required(),
	vendor: Joi.string()
...
export function validateSnapshot(snapshot: Snapshot, force = false) {
	...
	if (snapshot.backoffice !== version) {
		throw new InvalidPayloadError({
			reason: `Provided snapshot's backoffice version ${snapshot.backoffice} does not match the current instance's version ${version}. You can bypass this check by passing the "force" query parameter`,
		});
	}
