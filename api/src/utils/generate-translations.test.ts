...
describe('generateTranslations', () => {
	...
	beforeEach(() => {
		...
		// Default: return empty results for backoffice_fields queries (sort logic)
		tracker.on.select('backoffice_fields').response([]);
		tracker.on.update('backoffice_fields').response(0);
	...
	test('sorts new fields by parent collection field order when adding to existing translation collection', async () => {
		...
		tracker.on.select('backoffice_fields').responseOnce([
			{ field: 'id', sort: 1 },
			{ field: 'title', sort: 2 },
		...
		tracker.on.update('backoffice_fields').response(1);
	...
	test('shifts existing translation field sort values when new field is inserted before them', async () => {
		...
		tracker.on.update('backoffice_fields').response((rawQuery: any) => {
