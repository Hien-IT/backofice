import { InvalidQueryError } from '@backoffice/errors';
import type { DeepPartial } from '@backoffice/types';
...
/*
{
  ...
  collection: 'articles',
  aliasMap: {
    author: { alias: 'grenv', collection: 'backoffice_users' },
	'author.role': { alias: 'ljnsv', collection: 'backoffice_roles' },
  },
  relations: []
...
test('Returns correct alias for m2o', () => {
	const input: DeepPartial<ColPathProps> = {
		...
		collection: 'articles',
		aliasMap: {
			author: { alias: 'ljnsv', collection: 'backoffice_users' },
			'author.role': { alias: 'grenv', collection: 'backoffice_roles' },
		},
		relations: [
			{
				collection: 'articles',
				field: 'author',
				related_collection: 'backoffice_users',
				meta: null,
				schema: null,
			},
			{
				...
				field: 'role',
				related_collection: 'backoffice_roles',
				meta: null,
				schema: null,
			},
		],
	...
	expect(result.targetCollection).toBe('backoffice_roles');
});
...
test('Returns correct alias when there are multiple joins to the same table', () => {
	const input: DeepPartial<ColPathProps> = {
		...
		aliasMap: {
			author: { alias: 'ljnsv', collection: 'backoffice_users' },
			'author.role': { alias: 'grenv', collection: 'backoffice_roles' },
			'author.secondary_role': { alias: 'psgwn', collection: 'backoffice_roles' },
		},
		relations: [
			{
				collection: 'articles',
