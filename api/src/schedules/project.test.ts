...
test('Schedules synchronized job', async () => {
	tracker.on.select('backoffice_settings').response([{ project_status: undefined }]);
...
test('Sends report when pending', async () => {
	...
	expect(tracker.history.all[1]?.sql).toEqual('update "backoffice_settings" set "project_status" = ?');
