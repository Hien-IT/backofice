...
export async function up(knex: Knex): Promise<void> {
	const groups = await knex.select('*').from('backoffice_fields').where({ interface: 'group-standard' });
	...
	for (const field of raw) {
		await knex('backoffice_fields').update({ interface: 'group-raw' }).where({ id: field.id });
	}

	for (const field of detail) {
		await knex('backoffice_fields').update({ interface: 'group-detail' }).where({ id: field.id });
	}
}
