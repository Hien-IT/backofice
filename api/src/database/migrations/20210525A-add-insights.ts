...
export async function up(knex: Knex): Promise<void> {
	await knex.schema.createTable('backoffice_dashboards', (table) => {
	...
	await knex.schema.createTable('backoffice_panels', (table) => {
		table.uuid('id').primary().notNullable();
		table.uuid('dashboard').notNullable().references('id').inTable('backoffice_dashboards').onDelete('CASCADE');
		table.string('name');
...
export async function down(knex: Knex): Promise<void> {
	await knex.schema.dropTable('backoffice_panels');
	await knex.schema.dropTable('backoffice_dashboards');
}
