...
import type { RelationMeta } from '@backoffice/types';
...
export async function up(knex: Knex): Promise<void> {
	...
	if (corruptedRelations.length > 0) {
		logger.warn(
			`Encountered one or more corrupted relationships. Please check the following rows in "backoffice_relations": ${corruptedRelations.join(
				', ',
			)}`,
		);
	}
