...
import { resolvePackage } from '@backoffice/utils/node';
...
export async function up(knex: Knex): Promise<void> {
	await knex.schema.alterTable('backoffice_extensions', (table) => {
	...
	const installedExtensions = await knex.select('name').from('backoffice_extensions');
	...
	for (const { name } of installedExtensions) {
		...
		if (name.includes(':')) {
			await knex('backoffice_extensions').delete().where({ name });
		} else {
			const id = randomUUID();
			...
			await knex('backoffice_extensions').update({ id, source, folder: name }).where({ name });
	...
	for (const { name } of installedExtensions) {
		...
		await knex('backoffice_extensions')
			.update({
	...
	await knex.transaction(async (trx) => {
		await getHelpers(trx).schema.changePrimaryKey('backoffice_extensions', ['id']);
	});
...
export async function down(knex: Knex): Promise<void> {
	...
	const installedExtensions = await knex.select(['id', 'folder', 'bundle', 'source']).from('backoffice_extensions');
	...
	for (const { id, folder, bundle, source } of installedExtensions) {
		if (source === 'registry') {
			await knex('backoffice_extensions').delete().where({ id });
			continue;
		}
		...
		await knex('backoffice_extensions').update({ name }).where({ id });
	}

	await knex.transaction(async (trx) => {
		await getHelpers(trx).schema.changePrimaryKey('backoffice_extensions', ['name']);
	});
