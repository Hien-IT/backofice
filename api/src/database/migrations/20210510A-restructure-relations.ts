...
export async function up(knex: Knex): Promise<void> {
	...
	await knex('backoffice_relations').update({ one_deselect_action: 'nullify' });

	await helper.schema.changeToType('backoffice_relations', 'sort_field', 'string', {
		length: helper.schema.getColumnNameMaxLength(),
	});

	await helper.schema.changeToType('backoffice_relations', 'one_deselect_action', 'string', {
		nullable: false,
		default: 'nullify',
	});
}
