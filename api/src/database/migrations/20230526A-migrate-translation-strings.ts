...
export async function up(knex: Knex): Promise<void> {
	await knex.schema.createTable('backoffice_translations', (table) => {
	...
	const data = await knex.select('translation_strings', 'id').from('backoffice_settings').first();
	...
	if (data?.translation_strings && data?.id) {
		...
		for (const item of newTranslationStrings) {
			await knex('backoffice_translations').insert(item);
		}
	}
...
export async function down(knex: Knex): Promise<void> {
	const data = await knex.select('language', 'key', 'value').from('backoffice_translations');
	const settingsId = await knex.select('id').from('backoffice_settings').first();
	...
	if (settingsId?.id && data) {
		...
		await knex('backoffice_settings')
	...
	await knex.schema.dropTable('backoffice_translations');
}
