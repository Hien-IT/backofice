...
import type { Accountability, Permission } from '@backoffice/types';
import { processChunk, toBoolean } from '@backoffice/utils';
...
async function fetchRoleAccess(roles: string[], context: { knex: Knex }) {
	...
	const accessRows = await context
		.knex('backoffice_access')
		.select(
			'backoffice_policies.id',
			'backoffice_policies.admin_access',
			'backoffice_policies.app_access',
			'backoffice_policies.ip_access',
			'backoffice_policies.enforce_tfa',
		)
		.where('role', 'in', roles)
		.leftJoin('backoffice_policies', 'backoffice_policies.id', 'backoffice_access.policy');
...
export async function up(knex: Knex) {
	...
	if (await knex.schema.hasTable('backoffice_policies')) {
		return;
	}
	...
	await knex.schema.createTable('backoffice_policies', (table) => {
	...
	if (roles.length > 0) {
		await processChunk(roles, 100, async (chunk) => {
			await knex('backoffice_policies').insert(chunk);
		});
	}
	...
	try {
		...
		const foreignKeys = await inspector.foreignKeys('backoffice_permissions');

		const foreignConstraint =
			foreignKeys.find((foreign) => foreign.foreign_key_table === 'backoffice_roles' && foreign.column === 'role')
		...
		logger.warn('Failed to drop foreign key constraint on `role` column in `backoffice_permissions` table');
	}

	await knex('backoffice_permissions')
		.update({
			role: PUBLIC_POLICY_ID,
		})
		.whereNull('role');

	await knex('backoffice_permissions').update({
	...
	await knex.schema.alterTable('backoffice_permissions', (table) => {
		...
		table.foreign('policy').references('backoffice_policies.id').onDelete('CASCADE');
	});
	...
	// well, which would require the O2A type to exist in Backoffice which currently doesn't.
	// Shouldn't be the end of the world here, as we know we're only attaching policies to two other
	// collections.

	await knex.schema.createTable('backoffice_access', (table) => {
		table.uuid('id').primary();
		table.uuid('role').references('backoffice_roles.id').nullable().onDelete('CASCADE');
		table.uuid('user').references('backoffice_users.id').nullable().onDelete('CASCADE');
		table.uuid('policy').references('backoffice_policies.id').notNullable().onDelete('CASCADE');
		table.integer('sort');
	});
	...
	await processChunk(policyAttachments, 100, async (chunk) => {
		await knex('backoffice_access').insert(chunk);
	});

	await knex('backoffice_access').insert({
		id: randomUUID(),
		role: null,
		user: null,
...
export async function down(knex: Knex) {
	/////////////////////////////////////////////////////////////////////////////////////////////////
	// Reinstate access control fields on backoffice roles
	...
	// fetch all roles
	const roles: Array<{ id: string | null }> = await knex.select('id').from('backoffice_roles');
	...
	for (const role of roles) {
		...
		if (role.id !== null) {
			...
			await knex('backoffice_roles').update(roleAccess).where({ id: role.id });
		}
	...
	//  as soon as we reference backoffice_roles in backoffice_permissions again

	/////////////////////////////////////////////////////////////////////////////////////////////////
	// Drop policy attachments

	await knex.schema.dropTable('backoffice_access');
	...
	await processChunk(originalPermissions, 100, async (chunk) => {
		await knex('backoffice_permissions').delete(chunk);
	});

	// insert all v10 permissions
	await processChunk(rolePermissions, 100, async (chunk) => {
		await knex('backoffice_permissions').insert(chunk);
	});

	await knex.schema.alterTable('backoffice_permissions', (table) => {
		table.uuid('role').references('backoffice_roles.id').alter();
	...
	await knex.schema.dropTable('backoffice_policies');
}
