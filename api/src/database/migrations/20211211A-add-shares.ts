...
export async function up(knex: Knex): Promise<void> {
	...
	await knex.schema.createTable('backoffice_shares', (table) => {
		...
		table.string('item');
		table.uuid('role').references('id').inTable('backoffice_roles').onDelete('CASCADE');
	...
	await knex.schema.alterTable('backoffice_sessions', (table) => {
		table.dropColumn('data');
	});

	await knex.schema.alterTable('backoffice_sessions', (table) => {
		table.setNullable('user');
		table.uuid('share').references('id').inTable('backoffice_shares').onDelete('CASCADE');
	});
}
...
export async function down(knex: Knex): Promise<void> {
	...
	await knex.schema.dropTable('backoffice_shares');
}
