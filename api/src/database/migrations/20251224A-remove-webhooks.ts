...
export async function up(knex: Knex): Promise<void> {
	await knex.schema.dropTable('backoffice_webhooks');
}

export async function down(knex: Knex): Promise<void> {
	await knex.schema.createTable('backoffice_webhooks', (table) => {
		table.increments('id');
		...
		table.foreign('migrated_flow').references('backoffice_flows.id').onDelete('SET NULL');
	});
}
