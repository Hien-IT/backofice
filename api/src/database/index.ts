...
import type { DatabaseClient } from '@backoffice/types';
...
export async function isInstalled(): Promise<boolean> {
	...
	// The existence of a backoffice_collections table alone isn't a "proper" check to see if everything
	// is installed correctly of course, but it's safe enough to assume that this collection only
	// exists when Backoffice is properly installed.
	return await inspector.hasTable('backoffice_collections');
}
...
export async function validateMigrations(): Promise<boolean> {
	...
	try {
		...
		const completedVersions = (await database.select('version').from('backoffice_migrations')).map(
			({ version }) => version,
		);
