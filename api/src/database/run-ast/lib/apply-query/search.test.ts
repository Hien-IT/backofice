...
test(`Remove forbidden field(s) from search`, async () => {
	...
	applySearch(db as any, schema, queryBuilder, 'backoffice', 'test', {}, [
		{
			collection: 'test',
			action: 'read',
			fields: ['string'],
	...
	expect(rawQuery.bindings).toEqual(['%backoffice%']);
});
...
test(`Remove "conceal" field(s) from search irrespective of permissions`, async () => {
	...
	expect(rawQuery.bindings).toEqual(['%backoffice%', '%backoffice%']);
});
