...
import type { Filter, Permission, Query } from '@backoffice/types';
...
export function getDBQuery(
	...
	if (sortRecords) {
		...
		if (needsInnerQuery) {
			...
			if (hasMultiRelationalSort) {
				dbQuery.rowNumber(
					knex.ref('backoffice_row_number').toQuery(),
					knex.raw(`partition by ?? order by ${orderByString}`, [`${table}.${primaryKey}`, ...orderByFields]),
				);

				// Start order by with backoffice_row_number. The backoffice_row_number is derived from a window function that
				...
				// Since the only relevant result is the first row of this partition, adding the backoffice_row_number to the
				// order by here ensures that all rows with a backoffice_row_number = 1 show up first in the inner query result,
				// and are correctly truncated by the limit, but not earlier.
				orderByString = `?? asc, ${orderByString}`;
				orderByFields.unshift(knex.ref('backoffice_row_number'));
			}
	...
	if (sortRecords) {
		...
		if (hasMultiRelationalSort) {
			wrapperQuery.where('inner.backoffice_row_number', '=', 1);
