...
import { InvalidPayloadError, ServiceUnavailableError } from '@backoffice/errors';
...
export default async function createApp(): Promise<express.Application> {
	...
	if ((await isInstalled()) === false) {
		logger.error(`Database doesn't have Backoffice tables installed.`);
		process.exit(1);
	}
	...
	app.use((_req, res, next) => {
		res.setHeader('X-Powered-By', 'Backoffice');
		next();
	});
	...
	if (env['SERVE_APP']) {
		const adminPath = require.resolve('@backoffice/app');
		...
		const htmlWithVars = html
			...
			.replace('<!-- backoffice-embed-head -->', embeds.head)
			.replace('<!-- backoffice-embed-body -->', embeds.body);
