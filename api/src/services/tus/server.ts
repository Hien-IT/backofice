...
import type { Driver, TusDriver } from '@backoffice/storage';
import { supportsTus } from '@backoffice/storage';
import type { Accountability, File, SchemaOverview } from '@backoffice/types';
...
export async function createTusServer(context: Context): Promise<[Server, () => void]> {
	...
	const server = new Server({
		...
		async onUploadFinish(_req: any, upload) {
			...
			const service = new ItemsService<File>('backoffice_files', {
				schema,
			});
			...
			return {
				headers: {
					'Backoffice-File-Id': upload.metadata!['id']!,
				},
			};
		},
