...
import type { TusDriver } from '@backoffice/storage';
import type { Accountability, ChunkedUploadContext, File, SchemaOverview } from '@backoffice/types';
...
export class TusDataStore extends DataStore {
	...
	public override async create(upload: Upload): Promise<Upload> {
		...
		const filesItemsService = new ItemsService<File>('backoffice_files', {
		...
		if ('folder' in fileData === false) {
			const settings = await knex.select('storage_default_folder').from('backoffice_settings').first();
	...
	public override async write(readable: stream.Readable, tus_id: string, offset: number): Promise<number> {
		...
		const sudoFilesItemsService = new ItemsService<File>('backoffice_files', {
			schema: this.schema,
		});
