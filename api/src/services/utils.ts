...
import { systemCollectionRows } from '@backoffice/system-data';
import type { AbstractServiceOptions, Accountability, PrimaryKey, SchemaOverview } from '@backoffice/types';
...
export class UtilsService {
	...
	async sort(collection: string, { item, to }: { item: PrimaryKey; to: PrimaryKey }): Promise<void> {
		const sortFieldResponse =
			(await this.knex.select('sort_field').from('backoffice_collections').where({ collection }).first()) ||
