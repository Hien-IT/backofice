import { InvalidPayloadError } from '@backoffice/errors';
import type { AbstractServiceOptions, PrimaryKey } from '@backoffice/types';
...
export class TFAService {
	...
	constructor(options: AbstractServiceOptions) {
		...
		this.itemsService = new ItemsService('backoffice_users', options);
	}
	...
	async verifyOTP(key: PrimaryKey, otp: string, secret?: string): Promise<boolean> {
		...
		const user = await this.knex.select('tfa_secret').from('backoffice_users').where({ id: key }).first();
	...
	async generateTFA(key: PrimaryKey, requiresPassword: boolean = true): Promise<Record<string, string>> {
		...
		const project = await this.knex.select('project_name').from('backoffice_settings').limit(1).first();
		...
		return {
			secret,
			url: authenticator.keyuri(accountName, project?.project_name || 'Backoffice', secret),
		};
	}

	async enableTFA(key: PrimaryKey, otp: string, secret: string): Promise<void> {
		const user = await this.knex.select('tfa_secret', 'provider').from('backoffice_users').where({ id: key }).first();
