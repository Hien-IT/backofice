...
import { ForbiddenError, InvalidPayloadError, LimitExceededError, UnprocessableContentError } from '@backoffice/errors';
import { describe, type DescribeOptions } from '@backoffice/extensions-registry';
import type { ApiOutput, BundleExtension, ExtensionManager, ExtensionSettings } from '@backoffice/types';
import type { AbstractServiceOptions, Accountability, DeepPartial, SchemaOverview } from '@backoffice/types';
import { isObject } from '@backoffice/utils';
...
export class ExtensionsService {
	...
	constructor(options: AbstractServiceOptions) {
		...
		this.extensionsItemService = new ItemsService('backoffice_extensions', {
			knex: this.knex,
			schema: this.schema,
	...
	private async checkBundleAndSyncStatus(trx: Knex, extensionId: string, extension: ApiOutput) {
		if (extension.bundle === null && extension.schema?.type === 'bundle') {
			...
			await trx('backoffice_extensions')
		...
		const hasEnabledChildren = !!(await trx('backoffice_extensions')
			.where({ bundle: parentId })
			.where({ enabled: true })
			.first());

		if (hasEnabledChildren) {
			await trx('backoffice_extensions').update({ enabled: true }).where({ id: parentId });
		} else {
			await trx('backoffice_extensions').update({ enabled: false }).where({ id: parentId });
		}
	}
}
