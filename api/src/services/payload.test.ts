...
import type { Accountability, Item, PayloadAction } from '@backoffice/types';
		...
		describe('transformers', () => {
			...
			describe('csv', () => {
				...
				test('Returns array values as is', async () => {
					...
					expect(result).toEqual(['test', 'backoffice']);
				});
				...
				test('Splits the CSV string', async () => {
					...
					expect(result).toMatchObject(['test', 'backoffice']);
				});
				...
				test('Saves array values as joined string', async () => {
					...
					expect(result).toBe('test,backoffice');
				});
