...
export class AssetsService {
	...
	async getAsset(
		 ...
		 * with a wrong type. In case of backoffice_files where id is a uuid, we'll have to verify the
		...
		if (!systemPublicKeys.includes(id) && this.accountability && this.accountability.admin !== true) {
			...
			if (!accessAllowed) {
				throw new ForbiddenError({
					reason: `You don't have permission to perform "read" for collection "backoffice_files" or it does not exist.`,
				});
			}
