...
import type { File, SchemaOverview } from '@backoffice/types';
...
describe('AssetsService', () => {
	...
	beforeEach(() => {
		tracker.on.select(/backoffice_settings/).response([{}]);
	});
	...
	describe('getAsset', () => {
		...
		describe('system public assets', () => {
			it('should not filter fields for system public assets (logo, favicon, etc.)', async () => {
				...
				tracker.reset();
				tracker.on.select(/backoffice_settings/).response([{ project_logo: logoFileId }]);
