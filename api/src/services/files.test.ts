...
import { ForbiddenError, InternalServerError, InvalidPayloadError, ServiceUnavailableError } from '@backoffice/errors';
import { Driver, StorageManager } from '@backoffice/storage';
...
vi.mock('@backoffice/env', async (importOriginal) => {
	const actual = await importOriginal<typeof import('@backoffice/env')>();
...
describe('Service / Files', () => {
	...
	describe('createOne', () => {
		...
		test('should throw ForbiddenError deferred when filename_disk is not unique', async () => {
			tracker.on
				.select('select "filename_disk" from "backoffice_files" where "filename_disk" = ?')
		...
		test('should normalize filename_disk path', async () => {
			tracker.on.select('select "filename_disk" from "backoffice_files" where "filename_disk" = ?').response([]);
	...
	describe('uploadOne', () => {
		...
		beforeEach(() => {
			...
			tracker.on.select('select "storage_default_folder" from "backoffice_settings"').response([]);
		...
		test('should set the `uploaded_on` field to the current date', async () => {
			tracker.on
				.select(
					'select "folder", "filename_download", "filename_disk", "title", "description", "metadata" from "backoffice_files" where "id" = ?',
				)
				.response(null);
		...
		describe('storage default behavior', () => {
			it('should default to the first STORAGE_LOCATIONS when storage is not provided', async () => {
				...
				tracker.on
					.select(
						'select "folder", "filename_download", "filename_disk", "title", "description", "metadata", "storage" from "backoffice_files" where "id" = ?',
					)
					.response(null);
	...
	describe('updateMany', () => {
		...
		test('should exclude updated file from uniqueness check', async () => {
			...
			tracker.on
				.select(`select "filename_disk" from "backoffice_files" where "filename_disk" = ? and not "id" = ?`)
				.response([]);
