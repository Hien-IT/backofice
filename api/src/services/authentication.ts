...
import {
	...
	UserSuspendedError,
} from '@backoffice/errors';
import type { AbstractServiceOptions, Accountability, LoginResult, SchemaOverview } from '@backoffice/types';
...
import type { BackofficeTokenPayload, Session, User } from '../types/index.js';
...
export class AuthenticationService {
	...
	async login(
		...
		if (allowedAttempts !== null) {
			...
			try {
				...
				if (error instanceof RateLimiterRes && error.remainingPoints === 0) {
					await this.knex('backoffice_users').update({ status: 'suspended' }).where({ id: user.id });
					...
					if (this.accountability) {
						...
						const payloadService = new PayloadService('backoffice_users', {
							accountability: this.accountability,
							knex: this.knex,
							schema: this.schema,
						});
		...
		const tokenPayload: BackofficeTokenPayload = {
			id: user.id,
			role: user.role,
		...
		if (!user.tfa_secret) {
			...
			const roleEnforcement = await this.knex
				.select('backoffice_policies.enforce_tfa')
				.from('backoffice_users')
				.leftJoin('backoffice_roles', 'backoffice_users.role', 'backoffice_roles.id')
				.leftJoin('backoffice_access', 'backoffice_roles.id', 'backoffice_access.role')
				.leftJoin('backoffice_policies', 'backoffice_access.policy', 'backoffice_policies.id')
				.where('backoffice_users.id', user.id)
				.where('backoffice_policies.enforce_tfa', true)
				.first();
		...
		await this.knex('backoffice_sessions').insert({
			token: refreshToken,
			user: user.id,
		...
		await this.knex('backoffice_sessions').delete().where('expires', '<', new Date());
		...
		await this.knex('backoffice_users').update({ last_access: new Date() }).where({ id: user.id });
	...
	async refresh(refreshToken: string, options?: Partial<{ session: boolean }>): Promise<LoginResult> {
		...
		const record = await this.knex
			...
			.from('backoffice_sessions AS s')
			.leftJoin('backoffice_users AS u', 's.user', 'u.id')
			.leftJoin('backoffice_shares AS d', 's.share', 'd.id')
		...
		if (record.user_id && record.user_status !== 'active') {
			await this.knex('backoffice_sessions').where({ token: refreshToken }).del();
		...
		if (options?.session) {
			...
			await this.knex('backoffice_sessions')
				.update({
		...
		if (record.user_id) {
			await this.knex('backoffice_users').update({ last_access: new Date() }).where({ id: record.user_id });
		}
	...
	private async updateStatefulSession(
		...
		const updatedSession = await this.knex('backoffice_sessions')
			.update(
				{
		...
		if (updatedSession.length === 0) {
			...
			const { next_token } = await this.knex('backoffice_sessions')
				.select('next_token')
	...
	async logout(refreshToken: string): Promise<void> {
		const record = await this.knex
			...
			.from('backoffice_sessions as s')
			.innerJoin('backoffice_users as u', 's.user', 'u.id')
			.where('s.token', refreshToken)
			.first();
		...
		if (record) {
			...
			await this.knex.delete().from('backoffice_sessions').where('token', refreshToken);
		}
	}
