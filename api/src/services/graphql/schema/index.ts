...
import type { GraphQLParams, SchemaOverview } from '@backoffice/types';
...
export const READ_ONLY = ['backoffice_activity', 'backoffice_revisions'];
		...
		if (readableCollections.length > 0) {
			schemaComposer.Query.addFields(
				readableCollections.reduce(
					(acc, collection) => {
						const collectionName = gql.scope === 'items' ? collection.collection : collection.collection.substring(11);
