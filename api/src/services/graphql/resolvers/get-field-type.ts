...
export function getFieldType(schemaComposer: SchemaComposer, schema: Schema, action: 'read' | 'write') {
	...
	const Field = schemaComposer.createObjectTC({
		name: `${prefix}backoffice_fields`,
	});

	if ('backoffice_fields' in schema.read.collections === false) {
		return Field;
	}
	...
	Field.addFields({
		...
		meta: schemaComposer.createObjectTC({
			name: `${prefix}backoffice_fields_meta`,
			fields: Object.values(schema.read.collections['backoffice_fields']!.fields).reduce(
				(acc, field) => {
					acc[field.field] = {
						type:
		...
		schema: schemaComposer.createObjectTC({
			name: `${prefix}backoffice_fields_schema`,
			fields: {
				name: GraphQLString,
				table: GraphQLString,
