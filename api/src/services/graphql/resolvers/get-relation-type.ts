...
export function getRelationType(schemaComposer: SchemaComposer, schema: Schema, action: 'read' | 'write') {
	...
	const Relation = schemaComposer.createObjectTC({
		name: `${prefix}backoffice_relations`,
	});

	if ('backoffice_relations' in schema.read.collections === false) {
		return Relation;
	}
	...
	Relation.addFields({
		...
		schema: schemaComposer.createObjectTC({
			name: `${prefix}backoffice_relations_schema`,
			fields: {
		...
		meta: schemaComposer.createObjectTC({
			name: `${prefix}backoffice_relations_meta`,
			fields: Object.values(schema.read.collections['backoffice_relations']!.fields).reduce(
				(acc, field) => {
					acc[field.field] = {
