...
export function getCollectionType(schemaComposer: SchemaComposer, schema: Schema, action: 'read' | 'write') {
	...
	const Collection = schemaComposer.createObjectTC({
		name: `${prefix}backoffice_collections`,
	});

	if ('backoffice_collections' in schema.read.collections === false) {
		return Collection;
	}
	...
	Collection.addFields({
		...
		meta: schemaComposer.createObjectTC({
			name: `${prefix}backoffice_collections_meta`,
			fields: Object.values(schema.read.collections['backoffice_collections']!.fields).reduce(
				(acc, field) => {
					acc[field.field] = {
						type:
		...
		schema: schemaComposer.createObjectTC({
			name: `${prefix}backoffice_collections_schema`,
			fields: {
				name: GraphQLString,
