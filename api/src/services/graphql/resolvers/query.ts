import type { Item, Query } from '@backoffice/types';
import { parseFilterFunctionPath } from '@backoffice/utils';
 ...
 * Backoffice' query structure which is then executed by the services.
 */
export async function resolveQuery(gql: GraphQLService, info: GraphQLResolveInfo): Promise<Partial<Item> | null> {
	let collection = info.fieldName;
	if (gql.scope === 'system') collection = `backoffice_${collection}`;
