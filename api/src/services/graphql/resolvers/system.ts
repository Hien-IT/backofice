...
import type { CollectionAccess, GraphQLParams } from '@backoffice/types';
...
export function injectSystemResolvers(
	...
	if ('backoffice_collections' in schema.read.collections) {
	...
	if ('backoffice_fields' in schema.read.collections) {
	...
	if ('backoffice_relations' in schema.read.collections) {
	...
	if ('backoffice_users' in schema.read.collections) {
		schemaComposer.Query.addFields({
			users_me: {
				type: ReadCollectionTypes['backoffice_users']!,
	...
	if ('backoffice_permissions' in schema.read.collections) {
		schemaComposer.Query.addFields({
			permissions_me: {
	...
	if ('backoffice_roles' in schema.read.collections) {
		schemaComposer.Query.addFields({
			roles_me: {
				type: ReadCollectionTypes['backoffice_roles']!.List,
				resolve: dedupeResolver(async (_, args, __, info) => {
					if (!gql.accountability?.user && !gql.accountability?.role) return null;
	...
	if ('backoffice_policies' in schema.read.collections) {
		schemaComposer.Query.addFields({
			policies_me_globals: {
	...
	if ('backoffice_users' in schema.update.collections && gql.accountability?.user) {
		schemaComposer.Mutation.addFields({
			update_users_me: {
				type: ReadCollectionTypes['backoffice_users']!,
				args: {
					data: toInputObjectType(UpdateCollectionTypes['backoffice_users']!),
				},
				resolve: async (_, args, __, info) => {
					...
					if ('backoffice_users' in ReadCollectionTypes) {
	...
	if ('backoffice_files' in schema.create.collections) {
		schemaComposer.Mutation.addFields({
			import_file: {
				type: ReadCollectionTypes['backoffice_files'] ?? GraphQLBoolean,
				args: {
					url: new GraphQLNonNull(GraphQLString),
					data: toInputObjectType(CreateCollectionTypes['backoffice_files']!).setTypeName('create_backoffice_files_input'),
				},
				resolve: async (_, args, __, info) => {
					...
					if ('backoffice_files' in ReadCollectionTypes) {
						const selections = replaceFragmentsInSelections(
							info.fieldNodes[0]?.selectionSet?.selections,
							info.fragments,
						);
	...
	if ('backoffice_users' in schema.create.collections) {
		schemaComposer.Mutation.addFields({
			users_invite: {
				type: GraphQLBoolean,
				args: {
