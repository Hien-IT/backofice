...
import type { ForeignKey, SchemaInspector } from '@backoffice/schema';
import { createInspector } from '@backoffice/schema';
import { systemRelationRows } from '@backoffice/system-data';
import type {
...
export class RelationsService {
	...
	constructor(options: AbstractServiceOptions) {
		...
		this.relationsItemService = new ItemsService('backoffice_relations', {
			knex: this.knex,
			schema: this.schema,
			// We don't set accountability here. If you have read access to certain fields, you are
			// allowed to extract the relations regardless of permissions to backoffice_relations. This
	...
	async createOne(relation: Partial<Relation>, opts?: MutationOptions): Promise<void> {
		...
		try {
			...
			await transaction(this.knex, async (trx) => {
				...
				const relationsItemService = new ItemsService('backoffice_relations', {
					knex: trx,
					schema: this.schema,
					// We don't set accountability here. If you have read access to certain fields, you are
					// allowed to extract the relations regardless of permissions to backoffice_relations. This
	...
	async deleteOne(collection: string, field: string, opts?: MutationOptions): Promise<void> {
		...
		try {
			await transaction(this.knex, async (trx) => {
				...
				if (existingRelation.meta) {
					await trx('backoffice_relations').delete().where({ many_collection: collection, many_field: field });
				}
	...
	/**
	 * Combine raw schema foreign key information with Backoffice relations meta rows to form final
	 * Relation objects
	 */
