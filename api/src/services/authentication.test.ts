import { InvalidCredentialsError, InvalidOtpError, ServiceUnavailableError } from '@backoffice/errors';
...
describe('Integration Tests', () => {
	...
	describe('Services / Authentication', () => {
		...
		const setupHappyPathTracker = () => {
			tracker.on.select('backoffice_users').responseOnce([mockUser]);
			tracker.on.select('backoffice_users').responseOnce([]);
			tracker.on.insert('backoffice_sessions').response([1]);
			tracker.on.delete('backoffice_sessions').response(1);
			tracker.on.update('backoffice_users').response([1]);
		};
		...
		describe('login', () => {
			...
			it('should throw InvalidCredentialsError when user status is not active', async () => {
				tracker.on.select('backoffice_users').responseOnce([{ ...mockUser, status: 'inactive' }]);
			...
			it('should throw InvalidCredentialsError when user provider does not match', async () => {
				tracker.on.select('backoffice_users').responseOnce([{ ...mockUser, provider: 'ldap' }]);
			...
			it('should suspend user and reset rate limiter when login attempts are exhausted', async () => {
				tracker.on.select('backoffice_users').responseOnce([mockUser]);
				tracker.on.update('backoffice_users').responseOnce([1]);
				...
				expect(updateHistory[0]?.sql).toMatch(/backoffice_users/);
			});
			...
			it('should throw InvalidOtpError when TFA is configured but no OTP is provided', async () => {
				tracker.on.select('backoffice_users').responseOnce([{ ...mockUser, tfa_secret: 'some-secret' }]);
