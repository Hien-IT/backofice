...
import { ForbiddenError, InvalidInviteError, InvalidPayloadError, RecordNotUniqueError } from '@backoffice/errors';
import type {
...
import { UserIntegrityCheckFlag } from '@backoffice/types';
import { getSimpleHash, toArray, validatePayload } from '@backoffice/utils';
...
export class UsersService extends ItemsService {
	constructor(options: AbstractServiceOptions) {
		super('backoffice_users', options);
	 ...
	 * backoffice_settings.auth_password_policy
	 */
	...
	private async clearUserSessions(userKeys: PrimaryKey[], excludeSession?: string): Promise<void> {
		if (excludeSession) {
			...
			await this.knex.from('backoffice_sessions').whereIn('user', userKeys).delete();
		}
	}
	...
	override async deleteMany(keys: PrimaryKey[], opts: MutationOptions = {}): Promise<PrimaryKey[]> {
		...
		// Manual constraint, see https://github.com/backoffice/backoffice/pull/19912
		await this.knex('backoffice_comments').update({ user_updated: null }).whereIn('user_updated', keys);
		await this.knex('backoffice_notifications').update({ sender: null }).whereIn('sender', keys);
		await this.knex('backoffice_versions').update({ user_updated: null }).whereIn('user_updated', keys);
	...
	async requestPasswordReset(email: string, url: string | null, subject?: string | null): Promise<void> {
		...
		const token = jwt.sign(payload, getSecret(), { expiresIn: '1d', issuer: 'backoffice' });
