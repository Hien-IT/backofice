...
import { InvalidPayloadError, InvalidProviderConfigError } from '@backoffice/errors';
import type {
...
export class DeploymentService extends ItemsService<DeploymentConfig> {
	constructor(options: AbstractServiceOptions) {
		super('backoffice_deployments', options);
	}
	...
	private async readConfig(provider: ProviderType): Promise<DeploymentConfig> {
		const internalService = new ItemsService<DeploymentConfig>('backoffice_deployments', {
			knex: this.knex,
			schema: this.schema,
			accountability: null,
		});
	...
	async syncWebhook(provider: ProviderType): Promise<void> {
		...
		const projectsService = new ItemsService<DeploymentProject>('backoffice_deployment_projects', {
			knex: this.knex,
			schema: this.schema,
			accountability: null,
		});
