...
import type { AbstractServiceOptions, Accountability, SchemaOverview } from '@backoffice/types';
import { toArray, toBoolean } from '@backoffice/utils';
...
export class ServerService {
	...
	async isSetupCompleted(): Promise<boolean> {
		return Boolean(await this.knex('backoffice_users').first());
	}
	...
	async health(): Promise<Record<string, any>> {
		...
		async function testCache(): Promise<Record<string, HealthCheck[]>> {
			...
			try {
				await cache!.set(`backoffice-health-${checkID}`, true, 5);
				await cache!.delete(`backoffice-health-${checkID}`);
			} catch (err: any) {
		...
		async function testRateLimiter(): Promise<Record<string, HealthCheck[]>> {
			...
			try {
				await rateLimiter.consume(`backoffice-health-${checkID}`, 1);
				await rateLimiter.delete(`backoffice-health-${checkID}`);
			} catch (err: any) {
		...
		async function testRateLimiterGlobal(): Promise<Record<string, HealthCheck[]>> {
			...
			try {
				await rateLimiterGlobal.consume(`backoffice-health-${checkID}`, 1);
				await rateLimiterGlobal.delete(`backoffice-health-${checkID}`);
			} catch (err: any) {
		...
		async function testStorage(): Promise<Record<string, HealthCheck[]>> {
			...
			for (const location of toArray(env['STORAGE_LOCATIONS'] as string)) {
				...
				try {
					await disk.write('backoffice-health-file', Readable.from([checkID]));
				} catch (err: any) {
