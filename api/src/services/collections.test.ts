...
import type { Accountability, Collection, FieldMutationOptions } from '@backoffice/types';
...
const schema = new SchemaBuilder()
	.collection('backoffice_collections', (c) => {
		c.field('collection').string().primary();
	})
	.collection('backoffice_fields', (c) => {
	...
	describe('Services / Collections', () => {
		describe('createOne', () => {
			...
			const invalidPayloads: Array<[any, string]> = [
				...
				[{ collection: 'backoffice_test' }, 'collection names start with "backoffice_"'],
			...
			test('should throw InvalidPayloadError for existing collection', async () => {
				tracker.on.select('backoffice_collections').response([{ collection: 'existing_collection' }]);
				const service = new CollectionsService({ knex: db, schema, accountability: null });
			...
			test('should create collection with schema and default primary key', async () => {
				tracker.on.select('backoffice_collections').response([]);
				const service = new CollectionsService({ knex: db, schema, accountability: null });
		...
		describe('readByQuery', () => {
			test('should read collections for admin users', async () => {
				...
				// Should have test_collection plus system collections (backoffice_*)
			...
			test('should respect DB_EXCLUDE_TABLES environment variable', async () => {
				...
				const { useEnv } = await import('@backoffice/env');
		...
		describe('updateOne', () => {
			...
			test('should update existing collection meta', async () => {
				tracker.on.select('backoffice_collections').response([{ collection: 'test_collection' }]);
