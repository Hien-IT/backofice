...
import type { Column, SchemaInspector } from '@backoffice/schema';
import { createInspector } from '@backoffice/schema';
import { isSystemField } from '@backoffice/system-data';
import type {
...
import { addFieldFlag, getRelations, toArray } from '@backoffice/utils';
...
export class FieldsService {
	...
	constructor(options: AbstractServiceOptions) {
		...
		this.itemsService = new ItemsService('backoffice_fields', options);
		this.payloadService = new PayloadService('backoffice_fields', options);
	...
	async readAll(collection?: string): Promise<Field[]> {
		...
		const nonAuthorizedItemsService = new ItemsService<FieldMeta, 'backoffice_fields'>('backoffice_fields', {
			knex: this.knex,
			schema: this.schema,
		});
		...
		const aliasQuery = this.knex.select<any[]>('*').from('backoffice_fields');
	...
	async readOne(collection: string, field: string): Promise<Record<string, any>> {
		...
		let fieldInfo = await this.knex.select('*').from('backoffice_fields').where({ collection, field }).first();
	...
	async createField(
		...
		try {
			const exists =
				...
				isNil(
					await this.knex.select('id').from('backoffice_fields').where({ collection, field: field.field }).first(),
				) === false;

			// Check if field already exists, either as a column, or as a row in backoffice_fields
			if (exists) {
			...
			await transaction(this.knex, async (trx) => {
				const itemsService = new ItemsService('backoffice_fields', {
					knex: trx,
					accountability: this.accountability,
					schema: this.schema,
				});
			...
			await transaction(this.knex, async (trx) => {
				...
				if (permissionRows.length > 0) {
					for (const permissionRow of permissionRows) {
						...
						await trx('backoffice_permissions')
