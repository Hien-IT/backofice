...
import { normalizePath, toArray, toBoolean } from '@backoffice/utils';
...
export class FilesService extends ItemsService<File> {
	constructor(options: AbstractServiceOptions) {
		super('backoffice_files', options);
	}
	...
	private async checkUniqueFilename(filename: string, excludeId?: PrimaryKey) {
		const query = this.knex.select('filename_disk').from('backoffice_files').where({ filename_disk: filename });
	...
	async uploadOne(
		...
		if ('folder' in payload === false) {
			const settings = await this.knex.select('storage_default_folder').from('backoffice_settings').first();
		...
		const sudoFilesItemsService = new ItemsService('backoffice_files', {
			knex: this.knex,
			schema: this.schema,
		});
