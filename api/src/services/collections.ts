...
import { ForbiddenError, InvalidPayloadError } from '@backoffice/errors';
import type { SchemaInspector } from '@backoffice/schema';
import { createInspector } from '@backoffice/schema';
import { type BaseCollectionMeta, systemCollectionRows } from '@backoffice/system-data';
import type {
...
import { addFieldFlag } from '@backoffice/utils';
...
export class CollectionsService {
	...
	async createOne(payload: RawCollection, opts?: FieldMutationOptions): Promise<string> {
		...
		if (payload.collection.startsWith('backoffice_')) {
			throw new InvalidPayloadError({ reason: `Collections can't start with "backoffice_"` });
		}
		...
		try {
			const existingCollections: string[] = [
				...((await this.knex.select('collection').from('backoffice_collections'))?.map(({ collection }) => collection) ??
					[]),
			...
			await transaction(this.knex, async (trx) => {
				if (payload.schema) {
					...
					/**
					 * Backoffice heavily relies on the primary key of a collection, so we have to make sure that
					...
					const fieldItemsService = new ItemsService('backoffice_fields', {
						knex: trx,
				...
				if (payload.meta) {
					const collectionsItemsService = new ItemsService('backoffice_collections', {
						knex: trx,
						accountability: this.accountability,
						schema: this.schema,
					});
	...
	async deleteOne(collectionKey: string, opts?: MutationOptions): Promise<string> {
		...
		try {
			...
			await transaction(this.knex, async (trx) => {
				...
				await trx('backoffice_collections').update({ group: null }).where({ group: collectionKey });
				...
				if (collectionToBeDeleted!.schema) {
					...
					await trx('backoffice_presets').delete().where('collection', '=', collectionKey);

					const revisionsToDelete = await trx
						.select('id')
						.from('backoffice_revisions')
						.where({ collection: collectionKey });
					...
					if (revisionsToDelete.length > 0) {
						...
						for (const keys of chunks) {
							await trx('backoffice_revisions').update({ parent: null }).whereIn('parent', keys);
						}
					}

					await trx('backoffice_revisions').delete().where('collection', '=', collectionKey);

					await trx('backoffice_activity').delete().where('collection', '=', collectionKey);
					await trx('backoffice_permissions').delete().where('collection', '=', collectionKey);
					await trx('backoffice_relations').delete().where({ many_collection: collectionKey });
						...
						await Promise.all(
							Object.keys(this.schema.collections[collectionKey]?.fields ?? {}).map(async (fieldKey) => {
								...
								for (const meta of collectionMetaUpdates) {
									await trx('backoffice_collections').update(meta.updates).where({ collection: meta.collection });
								}
							}),
						);
					}
					...
					for (const relation of a2oRelationsThatIncludeThisCollection) {
						...
						await trx('backoffice_relations')
