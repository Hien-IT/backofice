...
const METRICS_SYNC_PACKET = 'backoffice:metrics---data-sync';
...
export function createMetrics() {
	...
	const metricNamePrefix = env['METRICS_NAME_PREFIX'] ?? 'backoffice_';
		...
		if (isPM2) {
			try {
				...
				for (const app of apps) {
					if (app.pm_id === undefined || app.pid === 0 || app.name !== 'backoffice') {
						continue;
					}
