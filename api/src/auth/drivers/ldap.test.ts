...
vi.mock('@backoffice/env', () => ({
	useEnv: vi.fn().mockReturnValue({
		REFRESH_TOKEN_COOKIE_NAME: 'backoffice_refresh_token',
		SESSION_COOKIE_NAME: 'backoffice_session_token',
...
describe('LDAP Auth Driver', () => {
	...
	describe('getUserID - new user creation', () => {
		...
		it('should throw InvalidProviderError when user creation fails with RecordNotUnique', async () => {
			...
			const recordNotUniqueError = new RecordNotUniqueError({ collection: 'backoffice_users', field: 'email' });
	...
	describe('createLDAPAuthRouter', () => {
		...
		it('should set cookie in cookie mode', async () => {
			...
			expect(mockRes.cookie).toHaveBeenCalledWith('backoffice_refresh_token', 'test-refresh-token', expect.any(Object));
		});
		...
		it('should set session cookie in session mode', async () => {
			...
			expect(mockRes.cookie).toHaveBeenCalledWith('backoffice_session_token', 'test-access-token', expect.any(Object));
		});

		it('should throw InvalidPayloadError for invalid request body', async () => {
			const { InvalidPayloadError } = await import('@backoffice/errors');
			const router = createLDAPAuthRouter('ldap');
