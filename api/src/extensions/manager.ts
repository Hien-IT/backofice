...
import { HYBRID_EXTENSION_TYPES } from '@backoffice/constants';
import { useEnv } from '@backoffice/env';
import { APP_SHARED_DEPS } from '@backoffice/extensions';
import { generateExtensionsEntrypoint } from '@backoffice/extensions/node';
import DriverLocal from '@backoffice/storage-driver-local';
import type {
	ActionHandler,
	ApiExtension,
	BundleConfig,
...
import { isTypeIn, toBoolean } from '@backoffice/utils';
import { pathToRelativeUrl, processId } from '@backoffice/utils/node';
...
export class ExtensionManager {
	...
	private async registerSandboxedApiExtension(extension: ApiExtension | HybridExtension) {
		...
		await module.instantiate(context, (specifier) => {
			if (specifier !== 'backoffice:api') {
				throw new Error('Imports other than "backoffice:api" are prohibited in API extension sandboxes');
			}
