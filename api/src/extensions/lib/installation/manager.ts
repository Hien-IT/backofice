...
import { ErrorCode, isBackofficeError, ServiceUnavailableError } from '@backoffice/errors';
import { EXTENSION_PKG_KEY, ExtensionManifest } from '@backoffice/extensions';
import { download, type DownloadOptions } from '@backoffice/extensions-registry';
...
export class InstallationManager {
	...
	async install(versionId: string) {
		...
		try {
			...
			if (isBackofficeError(err, ErrorCode.ServiceUnavailable)) {
				throw err;
			}
