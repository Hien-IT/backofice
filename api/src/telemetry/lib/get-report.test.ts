...
test('Runs and returns basic counts', async () => {
	const mockItemCount = {
		backoffice_dashboards: 15,
		backoffice_files: 45,
		backoffice_flows: 60,
		backoffice_roles: 75,
		backoffice_shares: 90,
	};
	...
	expect(getItemCount).toHaveBeenCalledWith(mockDb, [
		...
		{ collection: 'backoffice_flows', where: ['status', '=', 'active'] },
		{ collection: 'backoffice_roles' },
		{ collection: 'backoffice_shares' },
	]);

	expect(report.dashboards).toBe(mockItemCount.backoffice_dashboards);
	expect(report.files).toBe(mockItemCount.backoffice_files);
	expect(report.flows).toBe(mockItemCount.backoffice_flows);
	expect(report.roles).toBe(mockItemCount.backoffice_roles);
	expect(report.shares).toBe(mockItemCount.backoffice_shares);
});
