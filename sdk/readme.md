...
```ts
const client = createBackoffice<Schema>('https://api.backoffice.io');
```
...
```ts
const client = createBackoffice<Schema>('https://api.backoffice.io').with(rest()).with(graphql());
...
```ts
const client = createBackoffice<Schema>('https://api.backoffice.io').with(rest()).with(authentication('json'));
...
```ts
const client = createBackoffice<Schema>('https://api.backoffice.io').with(rest()).with(staticToken('super-secure-token'));
...
The `realtime()` extension allows you to work with a Backoffice REST WebSocket.

Subscribing to updates:

```ts
const client = createBackoffice<Schema>('https://api.backoffice.io').with(
	realtime({
		authMode: 'public',
	}),
);
