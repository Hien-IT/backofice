...
/**
 * Creates a client to communicate with a Backoffice REST WebSocket.
 *
 * @param config The optional configuration.
 *
 * @returns A Backoffice realtime client.
 */
export function realtime(config: WebSocketConfig = {}) {
	return <Schema>(client: BackofficeClient<Schema>) => {
		...
		const debug = (level: keyof ConsoleInterface, ...data: any[]) =>
			config.debug && client.globals.logger[level]('[Backoffice SDK]', ...data);
