import type { BackofficeFile, BackofficeFolder } from '../../../schema/index.js';
...
export const readAssetRaw =
	<Schema>(key: BackofficeFile<Schema>['id'], query?: AssetsQuery): RestCommand<ReadableStream<Uint8Array>, Schema> =>
	() => {
...
export const readAssetBlob =
	<Schema>(key: BackofficeFile<Schema>['id'], query?: AssetsQuery): RestCommand<Blob, Schema> =>
	() => {
		throwIfEmpty(String(key), 'Key cannot be empty');
...
export const readAssetArrayBuffer =
	<Schema>(key: BackofficeFile<Schema>['id'], query?: AssetsQuery): RestCommand<ArrayBuffer, Schema> =>
	() => {
		throwIfEmpty(String(key), 'Key cannot be empty');
...
export const downloadFolderZip =
	<Schema, R extends keyof AssetResponse = 'raw'>(
		key: BackofficeFolder<Schema>['id'],
		options?: { output: R },
	): RestCommand<AssetResponse[R], Schema> =>
	() => {
		throwIfEmpty(String(key), 'Key cannot be empty');
