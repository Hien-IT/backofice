...
vi.mock('./backoffice-frame.ts', () => ({
	BackofficeFrame: vi.fn(function (this: { send: typeof mockSend; isAiEnabled: () => boolean }) {
		this.send = mockSend;
...
describe('EditableElement', () => {
	...
	it('parses editConfig from data-backoffice attribute', () => {
	...
	describe('setParentsHover()', () => {
		...
		beforeEach(() => {
			...
			parent.dataset['backoffice'] = 'collection:pages;item:1';
			child = document.createElement('div');
			child.dataset['backoffice'] = 'collection:articles;item:2';
		...
		it('marks parent overlay with parent-hover class when child is hovered', () => {
			...
			expect(document.querySelectorAll('.backoffice-visual-editing-rect-parent-hover')).toHaveLength(1);
		});
		...
		it('removes parent-hover class when child is unhovered', () => {
			...
			expect(document.querySelectorAll('.backoffice-visual-editing-rect-parent-hover')).toHaveLength(0);
		});
	});
	...
	describe('query()', () => {
		it.each([undefined, null])('returns all [data-backoffice] elements when called with %s', (elements) => {
		...
		it('returns element itself when it has data-backoffice', () => {
			const el = createEditableElement();
		...
		it('returns children with data-backoffice when parent lacks the attribute', () => {
			const parent = document.createElement('div');
			const child1 = document.createElement('div');
			child1.dataset['backoffice'] = 'collection:a;item:1';
			const child2 = document.createElement('div');
			child2.dataset['backoffice'] = 'collection:b;item:2';
	...
	describe('editAttrToObject()', () => {
		...
		it('trims whitespace from values', () => {
			const el = document.createElement('div');
			el.dataset['backoffice'] = 'collection: articles ; item: 123 ';
			const editable = new EditableElement(el);
		...
		it('ignores unknown keys', () => {
			const el = document.createElement('div');
			el.dataset['backoffice'] = 'collection:articles;item:1;invalid:value;foo:bar';
			const editable = new EditableElement(el);
		...
		it('handles empty/malformed pairs without throwing', () => {
			const el = document.createElement('div');
			el.dataset['backoffice'] = 'collection:articles;;item:1;';
			const editable = new EditableElement(el);
