...
/**
 * *Singleton* class to handle communication with Backoffice in parent frame.
 */
export class BackofficeFrame {
	private static SINGLETON?: BackofficeFrame;
	private static readonly ERROR_PARENT_NOT_FOUND = 'Error sending message to Backoffice in parent frame:';
	...
	constructor() {
		if (BackofficeFrame.SINGLETON) return BackofficeFrame.SINGLETON;
		BackofficeFrame.SINGLETON = this;
	...
	send(action: SendAction, data?: unknown) {
		try {
			...
			console.error(BackofficeFrame.ERROR_PARENT_NOT_FOUND, error);
			return false;
		}
	}
