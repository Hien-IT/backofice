...
import { BackofficeFrame } from './backoffice-frame.ts';
...
export class EditableElement {
	private static readonly DATASET = 'backoffice';
	...
	private onClickEdit() {
		new BackofficeFrame().send('edit', { key: this.key, editConfig: this.editConfig, rect: this.rect });
	}

	private onClickAddToContext(event: MouseEvent) {
		event.stopPropagation();
		new BackofficeFrame().send('addToContext', { key: this.key, editConfig: this.editConfig, rect: this.rect });
	}
