...
describe('BackofficeFrame', () => {
	let postMessageSpy: ReturnType<typeof vi.spyOn>;

	beforeEach(() => {
		(BackofficeFrame as any).SINGLETON = undefined;
		vi.clearAllMocks();
	...
	it('returns same singleton instance on second construction', () => {
		const a = new BackofficeFrame();
		const b = new BackofficeFrame();
		expect(a).toBe(b);
	});

	describe('connect()', () => {
		it('stores origin and sends connect action to parent', () => {
			const frame = new BackofficeFrame();
