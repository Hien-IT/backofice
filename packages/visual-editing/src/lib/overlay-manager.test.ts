...
describe('OverlayManager', () => {
	...
	afterEach(() => {
		document.getElementById('backoffice-visual-editing')?.remove();
		document.getElementById('backoffice-visual-editing-style')?.remove();
	});
	...
	describe('getGlobalOverlay', () => {
		it('creates a div with the correct id when none exists', () => {
			...
			expect(overlay.id).toBe('backoffice-visual-editing');
		});
		...
		it('returns the same element on repeated calls', () => {
			...
			expect(document.querySelectorAll('#backoffice-visual-editing').length).toBe(1);
		});
	});
	...
	it('appends a <style> element with the correct id to <head>', () => {
		...
		const style = document.getElementById('backoffice-visual-editing-style');
