...
import { BackofficeFrame } from './lib/backoffice-frame.ts';
...
vi.mock('./lib/backoffice-frame.ts', () => ({
	BackofficeFrame: vi.fn(),
}));
...
function createEditableElement(config: EditConfig = { collection: 'articles', item: 1 }) {
	...
	el.dataset['backoffice'] = setAttr(config);
	document.body.appendChild(el);
	return el;
}

function setupFrame(overrides?: { connect?: boolean; confirm?: boolean; permittedKeys?: string[] }) {
	vi.mocked(BackofficeFrame).mockImplementation(function (this: any) {
		Object.assign(this, {
...
describe('apply()', () => {
	it('returns undefined when connect() returns false', async () => {
		...
		expect(await apply({ backofficeUrl: 'https://backoffice.example.com' })).toBeUndefined();
	});
	...
	it('injects style into document.head', async () => {
		await apply({ backofficeUrl: 'https://backoffice.example.com' });
		expect(document.head.querySelector('#backoffice-visual-editing-style')).toBeInstanceOf(HTMLStyleElement);
	});
	...
	it('applies customClass only to the specified element overlay', async () => {
		...
		await apply({ backofficeUrl: 'https://backoffice.example.com', elements: el1, customClass: 'my-class' });

		expect(document.querySelectorAll('.my-class').length).toBe(1);
		expect(document.querySelector('.backoffice-visual-editing-overlay.my-class')).toBeInstanceOf(HTMLElement);
	});

	describe('returned object', () => {
		it('contains a remove/enable/disable functions', async () => {
			createEditableElement();
			const result = await apply({ backofficeUrl: 'https://backoffice.example.com' });
		...
		describe('remove()', () => {
			it('removes scoped elements, leaving others intact', async () => {
				...
				const result1 = await apply({ backofficeUrl: 'https://backoffice.example.com', elements: el1 });
				await apply({ backofficeUrl: 'https://backoffice.example.com', elements: el2 });

				result1?.remove();

				expect(document.querySelectorAll('.backoffice-visual-editing-overlay').length).toBe(1);
		...
		describe('disable() / enable()', () => {
			it('disable() hides overlay rect, enable() restores it', async () => {
				createEditableElement();
				const result = await apply({ backofficeUrl: 'https://backoffice.example.com' });
				const rect = document.querySelector('.backoffice-visual-editing-rect') as HTMLElement;
...
describe('remove()', () => {
	it('removes all editable elements and their overlays', async () => {
		...
		expect(document.querySelectorAll('.backoffice-visual-editing-overlay').length).toBe(0);
	});
});
