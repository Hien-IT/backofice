...
export async function apply({
	...
	const backofficeFrame = new BackofficeFrame();
	const connected = backofficeFrame.connect(backofficeUrl);
	if (!connected) return;

	const confirmed = await backofficeFrame.receiveConfirm();
	if (!confirmed) return;

	PageManager.onNavigation((data) => backofficeFrame.send('navigation', data));
	...
	if (itemsToCheck.length) backofficeFrame.send('checkFieldAccess', itemsToCheck);
