...
import { JAVASCRIPT_FILE_EXTS } from '@backoffice/constants';
import { EXTENSION_LANGUAGES } from '@backoffice/extensions';
...
describe('validate extension', async () => {
	...
	test('backoffice-config', async () => {
		// Should pass with initial, unmodified state
		await expect(runValidate('backoffice-config')).resolves.toEqual(
			expect.objectContaining({ stderr: expect.stringContaining('Extension is valid') }),
		);
	});
