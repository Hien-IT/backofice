...
import { BUNDLE_EXTENSION_TYPES, EXTENSION_TYPES, HYBRID_EXTENSION_TYPES } from '@backoffice/constants';
import type { ExtensionOptions } from '@backoffice/extensions';
import { EXTENSION_LANGUAGES, EXTENSION_PKG_KEY } from '@backoffice/extensions';
import type {
	ApiExtensionType,
	AppExtensionType,
	BundleExtensionType,
...
import { isIn } from '@backoffice/utils';
...
async function createBundleExtension({
	...
	const spinner = ora(chalk.bold('Scaffolding Backoffice extension...')).start();
...
function getPackageManifest(name: string, options: ExtensionOptions, deps: Record<string, string>) {
	const packageManifest: Record<string, any> = {
		...
		icon: 'extension',
		version: '1.0.0',
		keywords: ['backoffice', 'backoffice-extension', `backoffice-extension-${options.type}`],
		type: 'module',
		files: ['dist'],
		[EXTENSION_PKG_KEY]: options,
		scripts: {
			build: 'backoffice-extension build',
			dev: 'backoffice-extension build -w --no-minify',
			link: 'backoffice-extension link',
			validate: 'backoffice-extension validate',
		},
		devDependencies: deps,
	};

	if (options.type === 'bundle') {
		packageManifest['scripts']['add'] = 'backoffice-extension add';
	}
