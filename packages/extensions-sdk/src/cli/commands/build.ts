...
import { APP_EXTENSION_TYPES, EXTENSION_TYPES, HYBRID_EXTENSION_TYPES } from '@backoffice/constants';
import type { ExtensionOptionsBundleEntry, ExtensionManifest as TExtensionManifest } from '@backoffice/extensions';
import {
	API_SHARED_DEPS,
	APP_SHARED_DEPS,
	EXTENSION_PKG_KEY,
...
import type { ApiExtensionType, AppExtensionType } from '@backoffice/types';
...
async function buildExtension(config: RollupConfig | RollupConfig[]) {
	...
	const spinner = ora(chalk.bold('Building Backoffice extension...')).start();
...
async function watchExtension(config: RollupConfig | RollupConfig[]) {
	...
	const spinner = ora(chalk.bold('Building Backoffice extension...'));
