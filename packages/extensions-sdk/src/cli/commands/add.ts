...
import { EXTENSION_TYPES, HYBRID_EXTENSION_TYPES } from '@backoffice/constants';
import type {
	ExtensionOptions,
	ExtensionOptionsBundleEntry,
	ExtensionManifest as TExtensionManifest,
} from '@backoffice/extensions';
import { EXTENSION_LANGUAGES, EXTENSION_PKG_KEY, ExtensionManifest } from '@backoffice/extensions';
import type { NestedExtensionType } from '@backoffice/types';
import { isIn, isTypeIn } from '@backoffice/utils';
import { pathToRelativeUrl } from '@backoffice/utils/node';
...
export default async function add(options: AddOptions): Promise<void> {
	...
	if (!(await fse.pathExists(packagePath))) {
		log(`Current directory is not a valid Backoffice extension:`, 'error');
	...
	if (extensionOptions.type === 'bundle') {
		...
		const spinner = ora(chalk.bold('Modifying Backoffice extension...')).start();
		...
		const newExtensionManifest = {
			...extensionManifest,
			name: extensionName,
			keywords: ['backoffice', 'backoffice-extension', 'backoffice-extension-bundle'],
			[EXTENSION_PKG_KEY]: newExtensionOptions,
			devDependencies: {
				...extensionManifest.devDependencies,
				...(await getExtensionDevDeps(
