import type { Filter, Query } from '@backoffice/types';
...
describe('useItems', () => {
	...
	test('should append $thumbnail to fetched items when collection is backoffice_files', async () => {
		vi.mocked(mockApiGet).mockResolvedValue({
			data: {
				data: [
					{
		...
		const collection = ref('backoffice_files');
