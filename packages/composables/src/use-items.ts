import type { Filter, Item, Query } from '@backoffice/types';
import { getEndpoint, moveInArray } from '@backoffice/utils';
...
export function useItems(collection: Ref<string | null>, query: ComputedQuery): UsableItems {
	...
	async function getItems() {
		...
		try {
			 ...
			 * able to render out the backoffice_files collection (file library) using regular layouts
			 *
			 * Layouts expect the file to be a m2o of a `file` type, however, backoffice_files is the
			 ...
			 */
			if (collection.value === 'backoffice_files') {
