import { API_INJECT, EXTENSIONS_INJECT, SDK_INJECT, STORES_INJECT } from '@backoffice/constants';
import type { BackofficeClient, RestClient } from '@backoffice/sdk';
import type { AppExtensionConfigs, RefRecord } from '@backoffice/types';
import type { AxiosInstance } from 'axios';
import { inject } from 'vue';

/**
 * Vue composable that provides access to the global Backoffice stores through dependency injection.
 *
 * This composable injects the stores object that contains all the Pinia stores used throughout
 * the Backoffice application, including user store, permissions store, collections store, etc.
 *
 ...
 * @example
 * ```typescript
 * import { useStores } from '@backoffice/composables';
 *
 ...
 *       const permissionsStore = stores.usePermissionsStore();
 *       return permissionsStore.hasPermission('backoffice_files', 'create');
 *     });
 *
 ...
 * Backoffice API. It provides a convenient way to make HTTP requests from components and composables.
 *
 * @returns The injected Axios instance configured for Backoffice API communication
 * @throws Error if the API instance could not be found in the injection context
 *
 * @example
 * ```typescript
 * import { useApi } from '@backoffice/composables';
 *
 * export default defineComponent({
 *   setup() {
 *     const api = useApi();
...
/**
 * Vue composable that provides access to the Backoffice SDK client instance through dependency injection.
 *
 * This composable injects the configured Backoffice SDK client that provides a type-safe, modern API
 * for interacting with Backoffice. The SDK offers methods for CRUD operations, authentication, file
 * management, and more, with full TypeScript support and automatic type inference based on your schema.
 *
 * @template Schema - The TypeScript schema type for your Backoffice instance, defaults to `any`
 * @returns The injected Backoffice SDK client with REST client capabilities
 * @throws Error if the SDK instance could not be found in the injection context
 *
 * @example
 * ```typescript
 * import { useSdk } from '@backoffice/composables';
 *
 ...
 * ```
 */
export function useSdk<Schema extends object = any>(): BackofficeClient<Schema> & RestClient<Schema> {
	const sdk = inject<BackofficeClient<Schema> & RestClient<Schema>>(SDK_INJECT);
...
/**
 * Vue composable that provides access to the registered Backoffice extensions through dependency injection.
 *
 ...
 * and utilize custom functionality within the Backoffice application.
 *
 ...
 * @example
 * ```typescript
 * import { useExtensions } from '@backoffice/composables';
 *
 * export default defineComponent({
 *   setup() {
