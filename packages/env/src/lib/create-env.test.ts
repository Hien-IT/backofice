...
import { isBackofficeVariable } from '../utils/is-backoffice-variable.js';
...
vi.mock('../utils/is-backoffice-variable.js');
...
describe('File based configuration', () => {
	beforeEach(() => {
		...
		vi.mocked(isBackofficeVariable).mockImplementation((key) => {
			return key === 'PROCESS_FILE';
		});
...
test('Passthrough file variables that are not Backoffice configuration flags', () => {
	vi.mocked(readConfigurationFromFile).mockReturnValue({
		TEST_FILE: './test/path',
	});

	vi.mocked(isBackofficeVariable).mockImplementation(() => {
		return false;
	});
